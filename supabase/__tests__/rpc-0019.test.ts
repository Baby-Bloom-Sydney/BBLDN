// `int.rpc-0019` — the functional suite over the three **stage** write definers `0019` creates:
// `upsert_position()`, `upsert_connection()` and `upsert_placement()`.
//
// `0019`'s other three — `create_child_invite()`, `revoke_child_invite()` and `apply_payment_event()` — and
// the creator column are in `rpc-0019-app.test.ts` beside this file. One migration, two suites, split by
// subject rather than by file size: these three are the stage model's writes and share every fixture below.
//
