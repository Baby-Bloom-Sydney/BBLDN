...
const STEPS = [
  {
    label: "Connect",
    description: "Find your perfect nanny",
    state: "completed" as const,
  },
  {
    label: "Meet",
    description: "Get to know eachother",
          ...
          <div className="flex">
            {STEPS.map((step, i) => {
              ...
              const leftColor = isFirst
                ? "bg-transparent"
                : connectorColor(STEPS[i - 1].state);
              ...
              return (
                ...
                >
                  ...
                  <p className={`mt-3 ${labelClass}`}>{step.label}</p>
              ...
              return (
                <div key={step.label} className="flex gap-3">
                  <div className="flex w-7 shrink-0 flex-col items-center">
                    ...
                    {!isLast && (
                      <div
                        className={`w-0.5 flex-1 ${connectorColor(step.state)}`}
                      />
                    )}
                  </div>
                  <div
                  ...
                  >
                    ...
                    <p
                      className={`text-xs leading-relaxed ${step.state === "future" ? "text-slate-300" : "text-slate-500"}`}
                    >
                      {step.description}
                    </p>
                    {!isLast && (
                      <div
                        className={step.state === "current" ? "h-5" : "h-2"}
                      />
                    )}
                  </div>
                </div>
