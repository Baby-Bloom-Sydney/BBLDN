...
import { TIME_BRACKETS, BRACKET_KEYS } from "@/lib/timezone";
import {
  ...
  const options = [
    {
      ...
      display: formatStartWeek(thisMon),
    },
    {
      label: "Next week",
      value: nextMon.toISOString().split("T")[0],
      display: formatStartWeek(nextMon),
    },
    {
      label: "In 2 weeks",
      value: in2Mon.toISOString().split("T")[0],
      display: formatStartWeek(in2Mon),
    },
    ...
    <div className="rounded-lg bg-amber-50 border border-amber-200 p-3 space-y-2">
      ...
      <div className="grid grid-cols-5 gap-1 text-xs">
        ...
        {BRACKET_KEYS.map((b) => (
          <div
            key={b}
            className="text-center text-amber-600 font-medium truncate"
          >
        ...
        {dates.map((date) => {
          ...
          return (
            <div key={date} className="contents">
              <div className="text-amber-700 font-medium truncate pr-1">
                {label}
              </div>
              {BRACKET_KEYS.map((b) => (
...
export function ConnectionDetailPopup({
  ...
  const [selectedStartWeek, setSelectedStartWeek] = useState<string | null>(
    null,
  );
  ...
  const isPostIntro =
    ...
    stage === CONNECTION_STAGE.AWAITING_RESPONSE;
  const isTrial =
    stage === CONNECTION_STAGE.TRIAL_ARRANGED ||
    stage === CONNECTION_STAGE.TRIAL_COMPLETE;
  const isTrialPending =
    stage === CONNECTION_STAGE.TRIAL_ARRANGED &&
    intro.fillInitiatedBy === "nanny";
            ...
            {intro.otherPartyPhoto ? (
              ...
              <div className="flex h-12 w-12 items-center justify-center rounded-full bg-violet-100">
                <span className="text-lg font-semibold text-violet-600">
                  {intro.otherPartyName.replace(/^The\s+/i, "").charAt(0)}
                </span>
              </div>
            )}
                ...
                {matchData &&
                  (() => {
                    const displayScore = Math.round(
                      50 + (matchData.matchScore / 100) * 50,
                    );
                    const scoreLabel =
                    ...
                    return (
                      <div
                        className={`rounded-lg border px-2 py-0.5 font-semibold text-xs shrink-0 ${getScoreBadgeStyle(displayScore)}`}
                      >
                ...
                {intro.otherPartySuburb && (
                  <span className="flex items-center gap-1">
                    ...
                    {matchData?.distanceKm != null &&
                      ` (${matchData.distanceKm < 1 ? "<1" : matchData.distanceKm.toFixed(0)} km)`}
                  </span>
                )}
                {role === "parent" &&
                  intro.nannyPhoneShared &&
                  stage >= CONNECTION_STAGE.INTRO_SCHEDULED && (
          ...
          <div className="rounded-lg border border-red-200 bg-red-50 p-3 space-y-2">
            ...
            <div className="flex gap-2">
              ...
              <Button
                ...
                onClick={() =>
                  handleAction(
                    () => onRemoveConnection(intro.connectionId),
                    "Connection removed.",
                  )
                }
              >
          ...
          <AvailabilityGrid
            ...
            onConfirm={async (slots) => {
              ...
              if (!result.success) {
                ...
                setSuccessMessage(
                  "Accepted! The family will schedule a meet and greet.",
                );
                setTimeout(() => {
                  onOpenChange(false);
                  setSuccessMessage(null);
        ...
        {role === "nanny" && isRequest && view === "confirm-decline" && (
          <div className="space-y-3">
            ...
            <p className="text-sm text-slate-500">
              This family is interested in working with you. Declining will
              remove this connection request.
            </p>
            <div className="flex gap-2">
              <Button
                size="sm"
          ...
          intro.proposedTimes && (
            <ScheduleTimeGrid
              ...
              onConfirm={async (isoTime) => {
                if (onScheduleTime) {
                  await handleAction(
                    () => onScheduleTime(intro.connectionId, isoTime),
          ...
          <div className="space-y-4">
            ...
            {matchData.breakdown && (
              <div className="space-y-3">
                ...
                <ScoreBar
                  label="Schedule"
                  value={matchData.breakdown.schedule}
                />
                <ScoreBar
                  label="Location"
                  value={matchData.breakdown.location}
                />
              </div>
            )}
          ...
          <div className="space-y-4">
            ...
            <div className="space-y-1.5">
              {matchData.overQualifiedBonuses?.map((b) => (
                <p key={b} className="text-sm text-green-600">
                  + {b}
                </p>
              ))}
              {matchData.unmetRequirements?.map((r) => (
                <p key={r} className="text-sm text-amber-600">
                  - {r}
                </p>
              ))}
            </div>
          ...
          <>
            ...
            {matchData && (
              <div className="flex items-center gap-3">
                ...
                {matchData.tier !== "standard" &&
                  ((matchData.overQualifiedBonuses?.length ?? 0) > 0 ||
                    (matchData.unmetRequirements?.length ?? 0) > 0) && (
                    <button
            ...
            {intro.startDate &&
              stage >= CONNECTION_STAGE.OFFERED &&
              (intro.startDate === "tbc" ? (
                role === "parent" && onUpdateStartWeek ? (
                  <div className="rounded-lg border border-amber-200 bg-amber-50 p-3 space-y-2">
                    ...
                    <StartWeekPicker
                      ...
                      onSelect={(v) => {
                        setSelectedStartWeek(v);
                        if (v !== "custom") setCustomStartDate("");
                      }}
                    ...
                    <Button
                      ...
                      disabled={
                        submitting ||
                        !selectedStartWeek ||
                        (selectedStartWeek === "custom" && !customStartDate)
                      }
                      onClick={() => {
                        const startVal =
                          selectedStartWeek === "custom"
                            ? customStartDate
                            : selectedStartWeek!;
                        handleAction(
                          () => onUpdateStartWeek(intro.connectionId, startVal),
                          "Start week updated!",
                        );
                      }}
                    >
              ...
              <div className="space-y-3 border-t pt-4">
                <div className="rounded-lg bg-blue-50 px-3 py-2">
                  <p className="text-xs text-blue-700">
                    Accepting will lead to your phone number being shared with
                    this family once they schedule a meet and greet.
                  </p>
                </div>
                <div className="flex gap-2">
                  <Button
              ...
              <div className="border-t pt-4">
                <div className="rounded-lg bg-green-50 border border-green-200 p-3 space-y-2">
                  ...
                  <p className="text-xs text-green-700">
                    Please call {name} at the scheduled time.
                  </p>
                </div>
              </div>
            )}
            ...
            {role === "nanny" && isScheduled && (
              <div className="border-t pt-4">
                <div className="rounded-lg bg-green-50 border border-green-200 px-3 py-2 text-xs text-green-700">
                  Your meet and greet is booked! Call {name} to confirm the
                  location.
                </div>
              </div>
            )}
                ...
                <div className="space-y-2 border-t pt-4">
                  ...
                  <div className="space-y-1.5">
                    ...
                    <Button
                      ...
                      onClick={() =>
                        setSelectedOutcome(
                          selectedOutcome === "trial" ? null : "trial",
                        )
                      }
                    >
                    ...
                    {selectedOutcome === "trial" && (
                      <div className="space-y-2 pl-6">
                        <p className="text-xs text-slate-500">
                          Congratulations! When is your trial shift?
                        </p>
                        <div className="flex items-center gap-2">
                          <input
                            type="date"
                            ...
                            min={minTrialDate}
                            className="text-sm border rounded px-2 py-1 text-slate-700 flex-1"
                          />
                          <Button
                            size="sm"
                    ...
                    <Button
                      ...
                      onClick={() =>
                        setSelectedOutcome(
                          selectedOutcome === "hired" ? null : "hired",
                        )
                      }
                    >
                    ...
                    {(stage === CONNECTION_STAGE.INTRO_COMPLETE ||
                      ...
                      <Button
                        ...
                        onClick={() =>
                          handleAction(
                            () =>
                              onIntroOutcome(intro.connectionId, "awaiting"),
                            "Updated.",
                          )
                        }
                      >
                    ...
                    <Button
                      size="sm"
                      variant="outline"
                      className="w-full justify-start text-left whitespace-normal text-slate-500"
                      disabled={submitting}
                      onClick={() =>
                        handleAction(
                          () => onIntroOutcome(intro.connectionId, "not_hired"),
                          "Updated.",
                        )
                      }
                    >
                      ...
                      <span>
                        The family have let me know they won&apos;t be needing
                        my services
                      </span>
                    </Button>
                    ...
                    {(stage === CONNECTION_STAGE.INTRO_COMPLETE ||
                      ...
                      <Button
                        size="sm"
                        variant="ghost"
                        className="w-full justify-start text-left whitespace-normal text-slate-400"
                        disabled={submitting}
                        onClick={() =>
                          handleAction(
                            () =>
                              onIntroOutcome(intro.connectionId, "incomplete"),
                            "Noted.",
                          )
                        }
                      >
              ...
              <div className="space-y-2 border-t pt-4">
                ...
                <div className="space-y-1.5">
                  ...
                  <Button
                    ...
                    onClick={() =>
                      handleAction(
                        () => onTrialOutcome(intro.connectionId, "awaiting"),
                        "Updated.",
                      )
                    }
                  >
                  ...
                  <Button
                    ...
                    onClick={() =>
                      handleAction(
                        () => onTrialOutcome(intro.connectionId, "not_hired"),
                        "Updated.",
                      )
                    }
                  >
                    <XCircle className="h-4 w-4 mr-2 shrink-0" />
                    <span>
                      The family have let me know they won&apos;t be needing my
                      services
                    </span>
                  </Button>
                </div>
                ...
                <div className="space-y-2 border-t pt-4">
                  ...
                  <div className="space-y-1.5">
                    <Button
                      ...
                      onClick={() =>
                        setSelectedOutcome(
                          selectedOutcome === "p-trial" ? null : "p-trial",
                        )
                      }
                    >
                      <ClipboardCheck className="h-4 w-4 mr-2 text-cyan-600 shrink-0" />
                    ...
                    <Button
                      ...
                      onClick={() =>
                        setSelectedOutcome(
                          selectedOutcome === "p-hired" ? null : "p-hired",
                        )
                      }
                    >
                      <CheckCircle className="h-4 w-4 mr-2 text-green-600 shrink-0" />
                    ...
                    {selectedOutcome === "p-hired" && (
                      <div className="space-y-2 pl-6">
                        <p className="text-xs text-slate-500">
                          When would you like {name} to start?
                        </p>
                        <StartWeekPicker
                          selected={selectedStartWeek}
                          customDate={customStartDate}
                    ...
                    <Button
                      ...
                      onClick={() =>
                        handleAction(
                          () => onParentOutcome(intro.connectionId, "awaiting"),
                          "Updated.",
                        )
                      }
                    >
                    ...
                    <Button
                      ...
                      onClick={() =>
                        handleAction(
                          () =>
                            onParentOutcome(intro.connectionId, "not_hired"),
                          "Updated.",
                        )
                      }
                    >
              ...
              <div className="space-y-3 border-t pt-4">
                ...
                {intro.trialDate && (
                  <div className="rounded-lg bg-violet-50 border border-violet-200 px-3 py-2 text-sm text-violet-700 flex items-center gap-2">
                    ...
                    {new Date(intro.trialDate + "T00:00:00").toLocaleDateString(
                      "en-AU",
                  ...
                  <div className="space-y-2 pl-6">
                    ...
                    <div className="flex items-center gap-2">
                      ...
                      <Button
                        ...
                        onClick={() =>
                          handleAction(
                            () => onConfirmTrial(intro.connectionId, dateValue),
                            "Trial confirmed!",
                          )
                        }
                      >
                ...
                {onDeclineTrial && (
                  <Button
                    ...
                    onClick={() =>
                      handleAction(
                        () => onDeclineTrial(intro.connectionId),
                        "Updated.",
                      )
                    }
                  >
              ...
              <div className="space-y-2 border-t pt-4">
                ...
                <div className="space-y-1.5">
                  <Button
                    ...
                    onClick={() =>
                      setSelectedOutcome(
                        selectedOutcome === "pt-hired" ? null : "pt-hired",
                      )
                    }
                  >
                    <CheckCircle className="h-4 w-4 mr-2 text-green-600 shrink-0" />
                ...
                <div className="space-y-3 border-t pt-4">
                  ...
                  <Button
                    ...
                    onClick={() => {
                      ...
                      handleAction(
                        () => onConfirmPosition(intro.connectionId),
                        "Position confirmed!",
                      );
                    }}
                  >
            ...
            {role === "parent" &&
              ...
              intro.fillInitiatedBy === "nanny" && (
                <div className="space-y-3 border-t pt-4">
                  <p className="text-sm font-medium text-slate-700">
                    {name} has indicated they&apos;ve been selected. Confirm to
                    ...
                    <>
                      ...
                      <Button
                        ...
                        onClick={() => {
                          ...
                          handleAction(
                            () =>
                              onConfirmPlacement(intro.connectionId, startVal),
                            "Placement confirmed!",
                          );
                        }}
                      >
                  ...
                  {onRevertToAwaiting && !confirmReject && (
                    <Button
                      ...
                      onClick={() =>
                        handleAction(
                          () => onRevertToAwaiting(intro.connectionId),
                          "Updated — we'll let them know.",
                        )
                      }
                    >
                  ...
                  {onRejectHiredClaim && confirmReject && (
                    <div className="rounded-lg border border-red-200 bg-red-50 p-3 space-y-2">
                      <p className="text-xs text-red-700">
                        Are you sure? This will end your connection with {name}{" "}
                        and you will no longer be able to communicate through
                        Baby Bloom.
                      </p>
                      <div className="flex gap-2">
                        <Button
                        ...
                        <Button
                          ...
                          onClick={() =>
                            handleAction(
                              () => onRejectHiredClaim(intro.connectionId),
                              "Connection ended.",
                            )
                          }
                        >
            ...
            {isOffered && !intro.fillInitiatedBy && (
              <div className="border-t pt-4">
                <div className="rounded-lg bg-amber-50 px-4 py-3 text-sm text-amber-700">
                  This position is pending confirmation. Please contact Baby
                  Bloom if you need help.
                </div>
              </div>
            )}
            ...
            {role === "nanny" && isNotHired && (
              <div className="space-y-4 border-t pt-4">
                <p className="text-sm text-slate-600">
                  The {name} family has decided to go in a different direction.
                  This is completely normal and happens often — the right family
                ...
                {onDismissConnection && (
                  <Button
                    ...
                    onClick={() =>
                      handleAction(
                        () => onDismissConnection(intro.connectionId),
                        "Removed.",
                      )
                    }
                  >
