// `dfy-waves` (01 §4f, 09:00 London) — the pre-check engine's own sweep, owned by `matching` because `matching`
// owns the pre-check (R2; 03 §7.4).
//
// A standalone exported function rather than a connector method, which is the shape `admin.callDueSweep`
// already uses for a cron's inside: it composes two things that are both already on connectors, and adding a
// fifth method to `Matching` would mean a fifth implementation in the stub and in the registry default for no
...
// So a position this run fired is not in the next run's cohort, and nothing durable needs to remember the run.
//
...
// nannies a re-blast may reach, which is `matching`'s business and not a cron's. See the pin in
...
/** The cron is the actor, and it is the job's own name (03 §2.5 `SystemJobName` `autofire`). */
...
export async function sweepPrecheckWaves(
  ...
  let handled = 0;
  // One position that cannot be pre-checked must not cost the rest of the morning's cohort theirs: each is
  ...
  for (const positionId of waiting.value) {
    const fired = await matching.autofire(positionId as PositionId, SWEEP);
    if (fired.ok) {
      handled += 1;
      continue;
    }
    log.warn("dfy-waves: a position could not be pre-checked", {
      module: "matching",
      action: "dfy-waves",
      positionId,
      at: now,
  ...
  return {
    ...
    value: Object.freeze({
      handled,
      skipped: waiting.value.length - handled,
    }),
  };
}
