// Repo-level rules for the `vetting-providers` module (build-standard L1 / L3; 01 §2.3 / §2.5; 05 §7 rule 6): the
...
describe("vetting-providers — folder shape (01 §2.5)", () => {
...
describe("vetting-providers — one export per file (L1; 05 §7 rule 4)", () => {
...
describe("vetting-providers — allowed imports (01 §2.3)", () => {
  it.each(files.map(rel))("%s imports only what its row allows", (file) => {
    ...
    for (const target of imports) {
      if (
        target === "vetting-providers" ||
        target.startsWith("vetting-providers/")
      )
        continue;
