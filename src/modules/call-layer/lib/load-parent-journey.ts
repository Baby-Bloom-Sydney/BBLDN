...
// the `UserId`, and 02 §4 gives `parents` its own id beside `user_id`. Until `positions`' inside (1e) settles
// which key the read model takes — or resolves one from the other — the user id is passed through here, in one
...
export async function loadParentJourney(): Promise<ParentJourneyLoad> {
  const actor = await parentActor();
  if (!actor.ok) return { kind: "signed-out" };
  const owner = actor.value.id as string as ParentId;
  const steps = await positions.getJourneySteps(owner);
