...
const DAYS: ReadonlyArray<NannyWeekday> = [
  "monday",
  "tuesday",
  "wednesday",
  "thursday",
...
const BLOCKS: ReadonlyArray<NannyDayBlock> = [
  "morning",
  "midday",
  "afternoon",
  "evening",
...
export function AvailabilityGrid({
  ...
  readonly defaultValue?: NannyAvailability | null;
}) {
  ...
  const toggle = (day: NannyWeekday, block: NannyDayBlock): void =>
    setGrid((current) => {
      ...
      const next = blocks.includes(block)
            ...
            <tr>
              ...
              {BLOCKS.map((block) => (
                <th
                  key={block}
                  scope="col"
                  className="px-2 py-1 text-left font-medium text-slate-600"
                >
                  {title(block)}
                </th>
              ))}
          ...
          <tbody>
            {DAYS.map((day) => (
              <tr key={day} className="border-t border-slate-100">
                <th
                  scope="row"
                  className="py-1 pr-2 text-left font-medium text-slate-800"
                >
                  {title(day)}
                </th>
                {BLOCKS.map((block) => (
