// Repo-level rules for the `verification` module (build-standard L1 / L3; 01 §2.3 / §2.5; 05 §7 rule 6): the
...
describe("verification — folder shape (01 §2.5)", () => {
...
describe("verification — one export per file (L1; 05 §7 rule 4)", () => {
...
describe("verification — allowed imports (01 §2.3)", () => {
  it.each(files.map(rel))("%s imports only what its row allows", (file) => {
    ...
    for (const target of imports) {
      if (target === "verification" || target.startsWith("verification/"))
        continue;
