...
**Schema names are config too — and they are already generated, not copied here.** Table, view, column, RPC and
enum names belong to the same rule as brand and locale: no file should hold them as a literal it cannot be ported
away from. They are **not** restated in this module, and the reason is the rule itself. `src/modules/shared-types/
database.types.ts` is generated by `npm run types:generate` from the applied migration set and held to it by CI's
`types-drift`; `Query<AppDatabase>` (`shared-types/platform.ts`) makes every one of those names a **type** —
`from` takes a `ReadableName`, `select` / `eq` / `insert` / `update` take `keyof` the generated row, `rpc` takes an
`RpcName`. So for `src/modules/**` and `src/boot/**` the config layer for schema names exists, it is generated, and
renaming a column is: change the migration, `npm run types:generate`, `npm run typecheck` — the compiler then names
every site that has to move, and misses none. A constants file listing the same names would be a second copy,
hand-maintained, able to drift from the schema in both directions, and **weaker at the rename**: a renamed constant
still compiles at every use, because its value is only a string. Bucket names, which the generated types do not
carry, ARE here — `UPLOADS.buckets` with `BucketKey` as the closed union, which is where that boundary sits.
`scripts/ci/check-schema-names.mjs` extends the same catalogue to the legacy tree, whose Supabase clients carry no
`Database` generic and so check nothing.
...
<!-- audit
Last edited: 2026-09-24T19:40+10:00 — BB-LDN-Planner-070926/SCHEMA-1
Notes: SCHEMA-1 — added "Schema names are config too": BAI's ruling that table / column / view / RPC / enum names are configuration is answered by the GENERATED types plus `Query<AppDatabase>`, not by a constants copy in this module; buckets stay here because the generated types do not carry them. Measured: of 195 identifiers in this repo that the schema does not have, zero are in `src/modules/` or `src/boot/`.
Prior: 2026-09-19T13:05+10:00 — BB-LDN-Planner-070926/3d-prime
