// Repo-level rules for the `public-site` module (build-standard L1 / L3; 01 §2.3 / §2.5; 05 §7 rule 6): the
...
describe("public-site — folder shape (01 §2.5)", () => {
...
describe("public-site — one export per file (L1; 05 §7 rule 4)", () => {
...
describe("public-site — allowed imports (01 §2.3)", () => {
  it.each(files.map(rel))("%s imports only what its row allows", (file) => {
    ...
    for (const target of imports) {
      if (target === "public-site" || target.startsWith("public-site/"))
        continue;
