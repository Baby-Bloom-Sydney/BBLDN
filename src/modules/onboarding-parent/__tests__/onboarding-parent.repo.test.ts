// Repo-level rules for the `onboarding-parent` module (build-standard L1 / L3; 01 §2.3 / §2.5; 05 §7 rule 6): the
...
describe("onboarding-parent — folder shape (01 §2.5)", () => {
...
describe("onboarding-parent — one export per file (L1; 05 §7 rule 4)", () => {
...
describe("onboarding-parent — allowed imports (01 §2.3)", () => {
  it.each(files.map(rel))("%s imports only what its row allows", (file) => {
    ...
    for (const target of imports) {
      if (
        target === "onboarding-parent" ||
        target.startsWith("onboarding-parent/")
      )
        continue;
