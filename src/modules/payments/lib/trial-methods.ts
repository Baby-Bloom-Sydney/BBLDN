...
export function trialMethods(
  deps: PaymentsDeps,
): Pick<PurchasePath, "startTrial"> {
  return {
    startTrial: async (familyId, actor) => {
      ...
      if (!current.ok) return carryStoreError(current.error);
      if (isDfy(current.value))
        return fail("E_DFY_FAMILY", "A done-for-you family has no trial");
      if (current.value?.has_used_trial)
      ...
      if (started === null || started.trial_ends_at === null)
        return ok({ alreadyUsed: true as const });
      const fresh =
        current.value?.trial_started_at === null || current.value === null;
      ...
      await emitMoneyEvents(deps.events, {
        ...
        props: {
          path: "self-serve",
          reason: "trial",
          trialEndsAt: started.trial_ends_at,
        },
      });
