...
import { configureNannyAccountStore, configureNannyLeadStore } from "@/modules/onboarding-nanny";
...
export function wireNannyOnboarding(): PortWiring {
  ...
  configureNannyAccountStore(dbNannyAccountStore(auth.data, () => auth.getCurrentUserId()));
  return {
    port: "nanny-onboarding",
    binding: "db-nanny-leads (service scope) + db-nanny-account (0021 definers, session scope)",
  };
}
