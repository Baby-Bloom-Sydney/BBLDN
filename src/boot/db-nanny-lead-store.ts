...
const leadFromRow = (row: LeadRow): NannyLead => {
  ...
  return Object.freeze({
    ...
    hasEnhancedDbs:
      typeof qualifications.has_enhanced_dbs === "boolean"
        ? qualifications.has_enhanced_dbs
        : null,
    yearsExperience:
      typeof experience.years === "number" ? experience.years : null,
    ageGroups: Array.isArray(experience.age_groups)
      ? (experience.age_groups as ReadonlyArray<NannyAgeGroup>)
      : [],
    roleTypes: Array.isArray(preferences.role_types)
      ? (preferences.role_types as ReadonlyArray<NannyRoleType>)
      : [],
    availability:
      Object.keys(availability).length === 0
        ? null
        : (availability as NannyAvailability),
    rateBand:
      typeof salary.min_pence === "number" &&
      typeof salary.max_pence === "number"
        ? ({
            minPence: salary.min_pence,
            maxPence: salary.max_pence,
          } as NannyRateBand)
        : null,
...
const columnsFor = (
  ...
  lead_signals: {
    under_three: input.ageGroups.some(
      (g) => g === "babies" || g === "toddlers",
    ),
  },
  source: input.source,
  funnel_step: "S-X-15",
...
const patchColumns = (patch: NannyLeadPatch) => ({
  ...(patch.roleTypes === undefined
    ? {}
    : { preferences: { role_types: [...patch.roleTypes] } }),
  ...(patch.availability === undefined
    ? {}
    : { availability: patch.availability }),
  ...(patch.rateBand === undefined
    ? {}
    : {
        salary: {
          min_pence: patch.rateBand.minPence,
          max_pence: patch.rateBand.maxPence,
        ...
        await port.run(
          {
            ...
            exec: async (q) => {
              ...
              const existing = (await q
                .from("nanny_leads")
                .eq("email", input.email)
                .single()) as LeadRow | null;
              if (existing !== null && existing.converted_at !== null)
                return {
                  leadId: existing.id as LeadId,
                  state: "has-account" as const,
                };
              if (existing !== null) {
                await q
                  .from("nanny_leads")
                  .update(existing.id as Uuid, columnsFor(input) as never);
                return {
                  leadId: existing.id as LeadId,
                  state: "updated" as const,
                };
              }
              // The id is minted here (the column accepts one, as `parent_leads` does — 02 §4.7): the row's
              // identity is known before the statement returns, so the cookie is minted from what was written.
              const leadId = newId<LeadId>();
              await q
                .from("nanny_leads")
                .insert({ id: leadId, ...columnsFor(input) } as never);
              return { leadId, state: "created" as const };
            },
          },
          service,
        ),
        ...
        await port.run(
          {
            ...
            exec: async (q) => {
              await q
                .from("nanny_leads")
                .update(leadId as string as Uuid, patchColumns(patch) as never);
            },
          },
          service,
        ),
