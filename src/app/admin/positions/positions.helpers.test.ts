...
describe("tierLabel", () => {
  it("maps 'priority' to BAI's 'Advanced' label", () => {
    expect(tierLabel("priority")).toBe("Advanced");
  });
  it("maps every other tier to 'Standard'", () => {
    expect(tierLabel("standard")).toBe("Standard");
    expect(tierLabel("")).toBe("Standard");
  });
});

describe("dfyStateBadge", () => {
  it("renders 'Not activated' for the none state", () => {
    expect(dfyStateBadge({ kind: "none" })).toEqual({
      variant: "inactive",
      label: "Not activated",
      detail: null,
    });
  ...
  it("renders an active badge with the tier label + an expiry detail", () => {
    const b = dfyStateBadge({
      kind: "active",
      tier: "priority",
      expiresAt: "2026-06-10T00:00:00.000Z",
    });
    expect(b.variant).toBe("verified");
    expect(b.label).toBe("Advanced — active");
    expect(b.detail).toMatch(/^expires /);
  });

  it("renders an active badge with a null detail when there is no expiry", () => {
    const b = dfyStateBadge({
      kind: "active",
      tier: "standard",
      expiresAt: null,
    });
    expect(b.label).toBe("Standard — active");
    expect(b.detail).toBeNull();
  });

  it("renders an expired badge with an expired detail", () => {
    const b = dfyStateBadge({
      kind: "expired",
      tier: "standard",
      expiredAt: "2026-05-30T00:00:00.000Z",
    });
    expect(b.variant).toBe("inactive");
    expect(b.label).toBe("Standard — expired");
    expect(b.detail).toMatch(/^expired /);
  });

  it("renders a completed badge", () => {
    expect(dfyStateBadge({ kind: "completed", tier: "priority" })).toEqual({
      variant: "active",
  ...
  it("renders an ended_user badge", () => {
    expect(dfyStateBadge({ kind: "ended_user" })).toEqual({
      variant: "failed",
      label: "Ended",
  ...
  it("renders an ended_system badge with a filled detail when filledAt is set", () => {
    const b = dfyStateBadge({
      kind: "ended_system",
      filledAt: "2026-06-02T00:00:00.000Z",
    });
    expect(b.variant).toBe("active");
    expect(b.label).toBe("Filled");
    expect(b.detail).toMatch(/^filled /);
  });

  it("renders an ended_system badge with a null detail when filledAt is null", () => {
    const b = dfyStateBadge({ kind: "ended_system", filledAt: null });
    expect(b.label).toBe("Filled");
    expect(b.detail).toBeNull();
  });

  it("produces a non-empty label for every state deriveDfyState can return", () => {
    const states: DfyState[] = [
      deriveDfyState(dfyInput({ dfy_activated_at: null }), NOW),
      deriveDfyState(dfyInput({ dfy_expires_at: FUTURE }), NOW),
      deriveDfyState(dfyInput({ dfy_expires_at: PAST, dfyMatches: [] }), NOW),
      deriveDfyState(
        dfyInput({
          dfy_expires_at: PAST,
          dfy_tier: "priority",
          dfyMatches: interestedRows(10),
        }),
        NOW,
      ),
      deriveDfyState(dfyInput({ position_status: 50 }), NOW),
      deriveDfyState(
        dfyInput({ position_status: 30, filled_by_nanny_id: "n1" }),
        NOW,
      ),
    ];
