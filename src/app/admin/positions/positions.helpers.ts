...
// ─── DFY state → badge (shared by the positions table column + DfyPanel) ───

/** "Advanced" is BAI's internal label for the 'priority' tier; everything else reads "Standard". */
export function tierLabel(tier: string): string {
  return tier === "priority" ? "Advanced" : "Standard";
}

/**
 * Map a derived {@link DfyState} to a badge variant + label + optional detail line.
 * The table column renders `label` only; DfyPanel renders `label` + `detail`.
 */
export function dfyStateBadge(state: DfyState): {
