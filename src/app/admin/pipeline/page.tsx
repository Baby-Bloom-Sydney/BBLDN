...
const TABLE_NAMES: Record<string, string> = {
  ...
  pd: "Page Drop-off",
  pf: "Parent Pipeline",
  pc: "Position Connections",
  nc: "Nanny Connections",
  nv: "Nanny Verification",
  ni: "Identity Verification",
  nw: "WWCC Verification",
  pv: "Parent Verification",
  kn: "Key Nanny Metrics",
  kp: "Key Parent Metrics",
  df: "DFY Matchmaking",
  dc: "DFY Connections",
};
  ...
  return CONN_STAGES.map((stage, i) => {
    ...
    const filtered =
      threshold === -1
        ...
        : conns.filter((c) => c.connection_stage >= threshold);
    const [mn, mx] = ranges[i];
    const liveFiltered = active.filter(
      (c) => c.connection_stage >= mn && c.connection_stage <= mx,
    );
    return {
      label: stage.label,
...
function applyCustomFilters(
  ...
  if (customRange && (customRange.from || customRange.to)) {
    filtered = filtered.filter((r: any) => {
      ...
      if (
        customRange.from &&
        customRange.from !== "1970-01-01" &&
        ts < `${customRange.from}T00:00:00`
      )
        return false;
...
function buildCustomStages(
  ...
  return customEntries
    .map((entry, i) => {
      ...
      const records = applyCustomFilters(
        stage.records,
        stage.idKey,
        customRange,
        customCfg.active,
      ...
      if (stage.liveRecords) {
        const liveRecs = applyCustomFilters(
          stage.liveRecords,
          stage.idKey,
          customRange,
          customCfg.active,
        ...
        const ro = customRo[i];
        liveTotal = countWithOverride(
          liveRecs,
          stage.idKey,
          ro?.count || customCfg.count,
          ro,
        );
      }
...
async function fetchCohort(admin: any, range: DateRange) {
  ...
  const [
    ...
    wd(
      admin
        .from("page_visits")
        .select("visitor_id, referrer, page_path, created_at"),
    ),
    wd(
      admin
        .from("page_visits")
        .select("visitor_id, created_at")
        .or("page_path.like./apply/nanny%,page_path.eq./apply"),
    ),
    wd(
      admin
        .from("nannies")
        .select("id, user_id, verification_level, created_at, updated_at"),
    ),
    wd(
      admin
        .from("parents")
        .select("id, user_id, signup_source, created_at, updated_at"),
    ),
    wd(
      admin
        .from("nanny_placements")
        .select("id, nanny_id, parent_id, created_at"),
    ),
    wd(
      admin
        .from("nanny_positions")
        .select("id, parent_id, status, source, created_at")
        .eq("source", "parent"),
    ),
    wd(
      admin
        .from("nanny_leads")
        .select(
          "id, visitor_id, lead_status, funnel_step, highest_page_reached, created_at, updated_at",
        ),
    ),
  ]);
  return {
...
async function fetchShared(admin: any) {
  const [
    ...
    admin
      ...
      .select(
        "id, nanny_id, parent_id, position_id, connection_stage, created_at",
      ),
    admin
      .from("user_stats")
      .select(
    ...
    admin
      .from("verifications")
      .select(
        "user_id, identity_status, wwcc_status, identity_status_at, wwcc_status_at, verification_status, created_at",
      ),
    admin
      .from("dfy_match_notifications")
      .select(
        "id, position_id, nanny_id, status, wave, notified_at, viewed_at, responded_at, created_at",
      )
      .then((r: any) => r)
      .catch(() => ({ data: null })),
    admin
      .from("nanny_positions")
      .select(
        "id, parent_id, dfy_activated_at, dfy_tier, dfy_expires_at, source, created_at",
      )
    ...
    admin
      .from("user_profiles")
      .select("user_id")
      .ilike("email", "%babybloomsydney.com.au"),
  ...
  const testNannyIds = new Set(
    (allNanniesRes.data || [])
      .filter((n: any) => testUserIds.has(n.user_id))
      .map((n: any) => n.id as string),
  );
  const testParentIds = new Set(
    (allParentsRes.data || [])
      .filter((p: any) => testUserIds.has(p.user_id))
  ...
  const testPositionIds = new Set(
    rawDfyPositions
      .filter((p: any) => testParentIds.has(p.parent_id))
      .map((p: any) => p.id),
  );
  const testLeadIds = new Set(
    rawUserStats
      .filter((us: any) => testUserIds.has(us.user_id) && us.lead_id)
      .map((us: any) => us.lead_id as string),
  );

  // ── Filter all shared data to exclude test accounts ──
  return {
    connections: ((connectionsRes.data || []) as any[]).filter(
      (c: any) =>
        !testNannyIds.has(c.nanny_id) && !testParentIds.has(c.parent_id),
    ),
    userStats: rawUserStats.filter((us: any) => !testUserIds.has(us.user_id)),
    connectionStatsData: ((connectionStatsRes.data || []) as any[]).filter(
      (cs: any) =>
        !testNannyIds.has(cs.nanny_id) && !testParentIds.has(cs.parent_id),
    ),
    verificationsData: ((verificationsRes.data || []) as any[]).filter(
      (v: any) => !testUserIds.has(v.user_id),
    ),
    dfyNotifications: ((dfyNotificationsRes.data || []) as any[]).filter(
      (n: any) =>
        !testNannyIds.has(n.nanny_id) && !testPositionIds.has(n.position_id),
    ),
    dfyPositions: rawDfyPositions.filter(
      (p: any) => !testParentIds.has(p.parent_id),
    ),
    testUserIds,
    testNannyIds,
    testParentIds,
...
async function getPipelineData(
  ...
  const cohortCache = new Map<
    string,
    Awaited<ReturnType<typeof fetchCohort>>
  >();
  ...
  if (shared.testUserIds.size > 0) {
    for (const [key, data] of cohortCache.entries()) {
      cohortCache.set(key, {
        ...data,
        nannies: data.nannies.filter(
          (n: any) => !shared.testUserIds.has(n.user_id),
        ),
        parents: data.parents.filter(
          (p: any) => !shared.testUserIds.has(p.user_id),
        ),
        positions: data.positions.filter(
          (p: any) => !shared.testParentIds.has(p.parent_id),
        ),
        placements: data.placements.filter(
          (pl: any) =>
            !shared.testNannyIds.has(pl.nanny_id) &&
            !shared.testParentIds.has(pl.parent_id),
        ),
  ...
  const cohort = (section: keyof typeof eff) =>
    cohortCache.get(rk(eff[section]))!;
  ...
  const nfNannies = filterByActivity(
    nfRaw.nannies,
    "user_id",
    userLastActive,
    nfCfg.active,
  );
  const nfLeads = filterByActivity(
    nfRaw.leads,
    "id",
    leadLastActive,
    nfCfg.active,
  ...
  const nfCohortConn = connections.filter((c: any) =>
    nfNannyIdSet.has(c.nanny_id),
  );
  ...
  const nfConnFiltered = (filter: (c: any) => boolean) =>
    nfCohortConn.filter(filter);

  const nannyFlow = [
    {
      label: "Visited Apply",
      tooltip: "Unique visitors to the nanny application page",
      total: countWithOverride(
        nfApplyVisits,
        "visitor_id",
        nfCfg.count,
    ...
    {
      label: "Started Form",
      tooltip: "Began filling out the multi-step application",
      total: nfCnt(1, nfLeads, "id"),
      override: nfRo[1],
    },
    {
      label: "Submitted Form",
      tooltip: "Completed and submitted the application",
      total: nfCnt(
        2,
        nfLeads.filter((l: any) =>
    ...
    },
    {
      ...
      tooltip: "AI-generated nanny profile created from form data",
      total: nfCnt(
        3,
        nfLeads.filter((l: any) =>
          ["ai_generated", "converted"].includes(l.lead_status),
        ),
        "id",
      ),
      override: nfRo[3],
    },
    {
      ...
      tooltip: "Nanny account registered on the platform",
      total: nfCnt(
        4,
        nfLeads.filter((l: any) => l.lead_status === "converted"),
        "id",
      ),
      override: nfRo[4],
    },
    {
      ...
      tooltip: "All verification steps completed (ID + WWCC + references)",
      total: nfCnt(
        5,
        nfNannies.filter((n: any) => n.verification_level === 4),
        "user_id",
      ),
      override: nfRo[5],
    },
    {
      label: "Requested",
      tooltip: "Received at least one connection request from a parent",
      total: nfCnt(6, nfCohortConn, "nanny_id"),
      override: nfRo[6],
    },
    {
      label: "Accepted",
      tooltip: "Accepted a connection request",
      total: nfCnt(
        7,
        nfConnFiltered((c) => c.connection_stage >= 10),
        "nanny_id",
      ),
      override: nfRo[7],
    },
    {
      ...
      tooltip: "Meet and greet time confirmed with a parent",
      total: nfCnt(
        8,
        nfConnFiltered((c) => c.connection_stage >= 20),
        "nanny_id",
      ),
      override: nfRo[8],
    },
    {
      label: "Meet Completed",
      tooltip: "Attended a meet and greet",
      total: nfCnt(
        9,
        nfConnFiltered((c) => c.connection_stage >= 21),
        "nanny_id",
      ),
      override: nfRo[9],
    },
    {
      ...
      tooltip: "Trial period arranged with a parent",
      total: nfCnt(
        10,
        nfConnFiltered((c) => c.connection_stage >= 31),
        "nanny_id",
      ),
      override: nfRo[10],
    },
    {
      ...
      tooltip: "Completed a trial period",
      total: nfCnt(
        11,
        nfConnFiltered((c) => c.connection_stage >= 32),
        "nanny_id",
      ),
      override: nfRo[11],
    },
    {
      label: "Offered",
      tooltip: "Received a formal job offer",
      total: nfCnt(
        12,
        nfConnFiltered((c) => c.connection_stage >= 33),
        "nanny_id",
      ),
      override: nfRo[12],
    },
    {
      label: "Placements",
      tooltip: "Confirmed employment with a family",
      total: nfCnt(
        13,
        nfConnFiltered((c) => PLACED_STAGES.has(c.connection_stage)),
        "nanny_id",
      ),
      override: nfRo[13],
    },
  ...
  const nfUserStats = userStats.filter(
    (us: any) => us.user_type === "nanny" && nfNannyUserIdSet.has(us.user_id),
  );
  const nannyTimestamps: (number | null)[][] = nfUserStats.map((us: any) => {
    ...
    return [
      null, // Visited Apply
      leadTs, // Started Form
      ["applied", "ai_generated", "converted"].includes(us.lead_status)
        ? leadTs
        : null, // Submitted Form
  ...
  const pdLeads = filterByActivity(
    pdRaw.leads,
    "id",
    leadLastActive,
    pdCfg.active,
  ...
  const leadsWithPages = pdLeads.filter(
    (l: any) => l.highest_page_reached != null && l.highest_page_reached > 0,
  );
  const pageDropoff =
    leadsWithPages.length >= 3
      ? pageStages.map((s, i) => ({
          label: s.label,
          tooltip: s.tooltip,
          total: leadsWithPages.filter(
            (l: any) => l.highest_page_reached >= i + 1,
          ).length,
        }))
      : null;
  ...
  const pfParentsBase = sourceFilter
    ? pfRaw.parents.filter((p: any) => p.signup_source === sourceFilter)
    : pfRaw.parents;
  const pfParents = filterByActivity(
    pfParentsBase,
    "user_id",
    userLastActive,
    pfCfg.active,
  ...
  const pfPositions = pfRaw.positions.filter((p: any) =>
    pfParentIdSet.has(p.parent_id),
  );
  const pfCohortConn = connections.filter((c: any) =>
    pfParentIdSet.has(c.parent_id),
  );
  ...
  const pfConnFiltered = (filter: (c: any) => boolean) =>
    pfCohortConn.filter(filter);

  const parentFunnel = [
    {
      label: "Account Created",
      tooltip: "Parent registered an account",
      total: pfCnt(0, pfParents, "user_id"),
      override: pfRo[0],
    },
    {
      label: "Position Created",
      tooltip: "Posted a childcare position listing",
      total: pfCnt(1, pfPositions, "parent_id"),
      override: pfRo[1],
    },
    {
      label: "Request Sent",
      tooltip: "Sent a connection request to a nanny",
      total: pfCnt(2, pfCohortConn, "parent_id"),
      override: pfRo[2],
    },
    {
      label: "Accepted",
      tooltip: "Request accepted by a nanny",
      total: pfCnt(
        3,
        pfConnFiltered((c) => c.connection_stage >= 10),
        "parent_id",
      ),
      override: pfRo[3],
    },
    {
      label: "Meet Scheduled",
      tooltip: "Meet and greet time confirmed",
      total: pfCnt(
        4,
        pfConnFiltered((c) => c.connection_stage >= 20),
        "parent_id",
      ),
      override: pfRo[4],
    },
    {
      ...
      total: pfCnt(
        5,
        pfConnFiltered((c) => c.connection_stage >= 21),
        "parent_id",
      ),
      override: pfRo[5],
    },
    {
      label: "Trial Arranged",
      tooltip: "Trial period arranged with a nanny",
      total: pfCnt(
        6,
        pfConnFiltered((c) => c.connection_stage >= 31),
        "parent_id",
      ),
      override: pfRo[6],
    },
    {
      ...
      total: pfCnt(
        7,
        pfConnFiltered((c) => c.connection_stage >= 32),
        "parent_id",
      ),
      override: pfRo[7],
    },
    {
      label: "Offered",
      tooltip: "Made a formal job offer",
      total: pfCnt(
        8,
        pfConnFiltered((c) => c.connection_stage >= 33),
        "parent_id",
      ),
      override: pfRo[8],
    },
    {
      label: "Hired",
      tooltip: "Nanny confirmed — placement active",
      total: pfCnt(
        9,
        pfConnFiltered((c) => PLACED_STAGES.has(c.connection_stage)),
        "parent_id",
      ),
      override: pfRo[9],
    },
  ...
  const pfUserStats = userStats.filter(
    (us: any) => us.user_type === "parent" && pfParentUserIdSet.has(us.user_id),
  );
  ...
  const pfAllParents = filterByActivity(
    pfRaw.parents,
    "user_id",
    userLastActive,
    pfCfg.active,
  ...
  const parentSourceBreakdown = SIGNUP_SOURCES.map((src) => ({
    ...
    total: pfAllParents.filter((p: any) => p.signup_source === src.value)
      .length,
  })).filter((s) => s.total > 0);
  // Add "Unknown" for parents without a tracked source
  const knownSourceValues = new Set(SIGNUP_SOURCES.map((s) => s.value));
  const unknownCount = pfAllParents.filter(
    (p: any) => !p.signup_source || !knownSourceValues.has(p.signup_source),
  ).length;
  if (unknownCount > 0) {
    parentSourceBreakdown.push({
      label: "Unknown",
      tooltip: "No signup source recorded",
      total: unknownCount,
    });
  }
  ...
  const pcParents = filterByActivity(
    pcRaw.parents,
    "user_id",
    userLastActive,
    pcCfg.active,
  ...
  const pcPositions = pcRaw.positions.filter((p: any) =>
    pcParentIdSet.has(p.parent_id),
  );
  const pcParentsWithPos = new Set(pcPositions.map((p: any) => p.parent_id));
  const pcConns = connections.filter((c: any) =>
    pcParentIdSet.has(c.parent_id),
  );
  const pcUniqueParentsInConn = new Set(pcConns.map((c: any) => c.parent_id))
    .size;
  ...
  const parentConnCumulative = [
    {
      label: "Parents",
      tooltip: "Total parent accounts in this cohort",
      total: countWithOverride(pcParents, "user_id", pcCfg.count, pcRo[0]),
      override: pcRo[0],
    },
    {
      label: "Positions",
      tooltip: "Total position listings created",
      total: countWithOverride(pcPositions, "id", pcCfg.count, pcRo[1]),
      override: pcRo[1],
    },
    {
      label: "Connections",
      tooltip: "Total connection requests from parents",
      total: countWithOverride(pcConns, "id", pcCfg.count, pcRo[2]),
      override: pcRo[2],
    },
    ...buildConnCumulative(pcConns, "parent_id", pcCfg.count, pcRo, 3),
  ];
  const pcActiveConns = pcConns.filter(
    (c: any) => !TERMINAL_STAGES.has(c.connection_stage),
  );
  const parentConnLive = [
    {
      label: "Parents",
      tooltip: "Total parent accounts in this cohort",
      total: Math.max(0, pcParents.length - pcParentsWithPos.size),
      override: pcRo[0],
    },
    {
      label: "Positions",
      tooltip: "Total position listings created",
      total: pcPositions.length,
      override: pcRo[1],
    },
    {
      label: "Connections",
      tooltip: "Active connection requests",
      total: pcActiveConns.length,
      override: pcRo[2],
    },
  ...
  const pcUserStats = userStats.filter(
    (us: any) => us.user_type === "parent" && pcParentUserIdSet.has(us.user_id),
  );
  const parentConnTimestamps: (number | null)[][] = pcUserStats.map(
    (us: any) => [
      null, // Parents
  ...
  const ncNannies = filterByActivity(
    ncRaw.nannies,
    "user_id",
    userLastActive,
    ncCfg.active,
  ...
  const ncUniqueNanniesInConn = new Set(ncConns.map((c: any) => c.nanny_id))
    .size;
  ...
  const nannyConnCumulative = [
    {
      label: "Nannies",
      tooltip: "Total nanny accounts in this cohort",
      total: countWithOverride(ncNannies, "user_id", ncCfg.count, ncRo[0]),
      override: ncRo[0],
    },
  ...
  const nannyConnLive = [
    {
      label: "Nannies",
      tooltip: "Total nanny accounts in this cohort",
      total: Math.max(0, ncNannies.length - ncUniqueNanniesInConn),
      override: ncRo[0],
    },
  ...
  const ncUserStats = userStats.filter(
    (us: any) => us.user_type === "nanny" && ncNannyUserIdSet2.has(us.user_id),
  );
  const nannyConnTimestamps: (number | null)[][] = ncUserStats.map(
    (us: any) => [
      null, // Nannies
      toMs(us.first_connection_at), // Requested
      toMs(us.first_accepted_at), // Accepted
  ...
  const parentTerminalStages = Object.entries(TERMINAL_LABELS)
    .map(([stage, label]) => ({
      ...
      total: new Set(
        connections
          .filter((c: any) => c.connection_stage === parseInt(stage))
  ...
  const nannyTerminalStages = Object.entries(TERMINAL_LABELS)
    .map(([stage, label]) => ({
      ...
      total: new Set(
        connections
          .filter((c: any) => c.connection_stage === parseInt(stage))
          .map((c: any) => c.nanny_id),
      ).size,
    }))
    .filter((s) => s.total > 0);

  // ── Secondary stats (global cohort) ──
  const supplyCount = g.nannies.filter(
    (n: any) => n.verification_level >= 2,
  ).length;
  const demandCount = g.positions.filter(
    (p: any) => p.status === "active" || p.status === "open",
  ).length;

  const parentCreateMap = new Map(
    g.parents.map((p: any) => [p.id, new Date(p.created_at).getTime()]),
  );
  ...
  for (const pl of g.placements) {
    ...
    if (parentTs)
      daysArr.push(
        Math.round((new Date(pl.created_at).getTime() - parentTs) / 86400000),
      );
  }
  daysArr.sort((a, b) => a - b);
  const medianDays =
    daysArr.length > 0 ? daysArr[Math.floor(daysArr.length / 2)] : null;
  ...
  for (const conn of connections) {
    if (conn.position_id)
      posConnMap.set(
        conn.position_id,
        (posConnMap.get(conn.position_id) || 0) + 1,
      );
  }
  const posConnCounts = Array.from(posConnMap.values());
  const avgConnsPerPos =
    posConnCounts.length > 0
      ? Math.round(
          (posConnCounts.reduce((a, b) => a + b, 0) / posConnCounts.length) *
            10,
        ) / 10
      : null;
  ...
  for (const conn of connections) {
    if (conn.position_id && PLACED_STAGES.has(conn.connection_stage))
      placedPositionIds.add(conn.position_id);
  }
  const fillRate =
    g.positions.length > 0
      ? Math.round((placedPositionIds.size / g.positions.length) * 100)
      : null;

  // ── Nanny Verification Levels (uses nv cohort) ──
  const nvRaw = cohort("nv");
  const nvCfg = configs.nv || { count: "unique" as const };
  const nvNannies = filterByActivity(
    nvRaw.nannies,
    "user_id",
    userLastActive,
    nvCfg.active,
  ...
  const nannyVerifLevelsCumulative = [
    {
      label: "Nannies",
      tooltip: "Total nanny accounts",
      total: nvNannies.length,
    },
    {
      label: "ID Verified",
      tooltip: "Identity document verified (Level 1)",
      total: nvNannies.filter((n: any) => n.verification_level >= 1).length,
    },
    {
      label: "WWCC Verified",
      tooltip: "Working With Children Check verified (Level 2)",
      total: nvNannies.filter((n: any) => n.verification_level >= 2).length,
    },
    {
      label: "Provisional",
      tooltip: "Basic verifications complete (Level 3)",
      total: nvNannies.filter((n: any) => n.verification_level >= 3).length,
    },
    {
      label: "Fully Verified",
      tooltip: "All verification steps completed (Level 4)",
      total: nvNannies.filter((n: any) => n.verification_level >= 4).length,
    },
  ];
  ...
  const niNannies = filterByActivity(
    niRaw.nannies,
    "user_id",
    userLastActive,
    niCfg.active,
  ...
  const niNannyVerifs = verificationsData.filter((v: any) =>
    niNannyUserIdSet.has(v.user_id),
  );

  const nannyIdentityCumulative = [
    {
      label: "Nannies",
      tooltip: "Nannies with a verification record",
      total: niNannyVerifs.length,
    },
    {
      label: "Initiated",
      tooltip: "Started identity verification",
      total: niNannyVerifs.filter(
        (v: any) => v.identity_status !== "not_started",
      ).length,
    },
    {
      label: "Processing",
      tooltip: "Identity document being processed",
      total: niNannyVerifs.filter((v: any) =>
        IDENTITY_PAST_PENDING.includes(v.identity_status),
      ).length,
    },
    {
      label: "Outcome",
      tooltip: "Processing complete — result received",
      total: niNannyVerifs.filter((v: any) =>
        IDENTITY_OUTCOMES.includes(v.identity_status),
      ).length,
    },
    {
      label: "Verified",
      tooltip: "Identity successfully verified",
      total: niNannyVerifs.filter((v: any) => v.identity_status === "verified")
        .length,
    },
  ];
  ...
  const nwNannies = filterByActivity(
    nwRaw.nannies,
    "user_id",
    userLastActive,
    nwCfg.active,
  ...
  const nwNannyVerifs = verificationsData.filter((v: any) =>
    nwNannyUserIdSet.has(v.user_id),
  );
  ...
  const nannyWwccCumulative = [
    {
      label: "Nannies",
      tooltip: "Nannies with a verification record",
      total: nwNannyVerifs.length,
    },
    {
      label: "Initiated",
      tooltip: "Started WWCC process",
      total: nwNannyVerifs.filter((v: any) => v.wwcc_status !== "not_started")
        .length,
    },
    {
      label: "Submitted",
      tooltip: "WWCC application submitted",
      total: nwNannyVerifs.filter((v: any) =>
        WWCC_PAST_APP_PENDING.includes(v.wwcc_status),
      ).length,
    },
    {
      label: "Processing",
      tooltip: "WWCC being processed",
      total: nwNannyVerifs.filter((v: any) =>
        WWCC_PAST_PENDING.includes(v.wwcc_status),
      ).length,
    },
    {
      label: "Outcome",
      tooltip: "Processing complete — result received",
      total: nwNannyVerifs.filter((v: any) =>
        WWCC_OUTCOMES.includes(v.wwcc_status),
      ).length,
    },
    {
      label: "Verified",
      tooltip: "WWCC document verified and current",
      total: nwNannyVerifs.filter((v: any) => v.wwcc_status === "doc_verified")
        .length,
    },
  ];
  const nannyWwccLive = [
  ...
  const pvParents = filterByActivity(
    pvRaw.parents,
    "user_id",
    userLastActive,
    pvCfg.active,
  ...
  const pvParentVerifs = verificationsData.filter((v: any) =>
    pvParentUserIds.has(v.user_id),
  );

  const parentIdentityCumulative =
    pvParentVerifs.length > 0
      ? [
          {
            label: "Parents",
            tooltip: "Parents with a verification record",
            total: pvParentVerifs.length,
          },
          {
            label: "Initiated",
            tooltip: "Started identity verification",
            total: pvParentVerifs.filter(
              (v: any) => v.identity_status !== "not_started",
            ).length,
          },
          {
            label: "Processing",
            tooltip: "Identity document being processed",
            total: pvParentVerifs.filter((v: any) =>
              IDENTITY_PAST_PENDING.includes(v.identity_status),
            ).length,
          },
          {
          ...
          },
          {
            ...
            tooltip: "Identity successfully verified",
            total: pvParentVerifs.filter(
              (v: any) => v.identity_status === "verified",
            ).length,
          },
        ]
      : null;
  const parentIdentityLive =
    pvParentVerifs.length > 0
      ? [
          {
            label: "Parents",
  ...
  const pageCategories: {
    ...
    {
      label: "Home",
      tooltip: "Visits to the homepage (/)",
      match: (p) => p === "/",
    },
    {
      label: "For Nannies",
      tooltip: "Visits to /childcare-professionals",
      match: (p) => p === "/for-nannies" || p === "/childcare-professionals",
    },
    {
      label: "Browse Nannies",
      tooltip: "Visits to /nannies and nanny profiles",
      match: (p) => p === "/nannies" || p.startsWith("/nannies/"),
    },
    {
      label: "Quick Match Results",
      tooltip: "Visits to /matchmaking/results (from quick match widget)",
      match: (p) => p.startsWith("/matchmaking/results"),
    },
    {
      label: "Matchmaking Form",
      tooltip: "Visits to /matchmaking/onboarding (advanced matchmaking)",
      match: (p) => p.startsWith("/matchmaking/onboarding"),
    },
    {
      label: "Matchmaking Signup",
      tooltip: "Visits to /matchmaking/signup (signup after matching)",
      match: (p) => p.startsWith("/matchmaking/signup"),
    },
    {
      label: "How It Works",
      tooltip: "Visits to /how-it-works",
      match: (p) => p === "/how-it-works",
    },
    {
      label: "Pricing",
      tooltip: "Visits to /pricing",
      match: (p) => p === "/pricing",
    },
    {
      label: "Apply Page",
      tooltip: "Visits to /apply",
      match: (p) => p.startsWith("/apply/nanny") || p === "/apply",
    },
    {
      label: "Signup Pages",
      tooltip: "Visits to /signup pages",
      match: (p) => p.startsWith("/signup"),
    },
    {
      label: "Login",
      tooltip: "Visits to /login",
      match: (p) => p === "/login",
    },
    {
      label: "Nanny Profiles",
      tooltip: "Visits to individual nanny profile pages (shared to Facebook)",
      match: (p) => /^\/nannies\/[^/]+/.test(p),
    },
    {
      label: "Position Pages",
      tooltip: "Visits to shared position pages",
      match: (p) => /^\/position\/[^/]+/.test(p),
    },
  ];
  ...
  const pageTraffic = pageCategories.map(({ label, tooltip, match }) => {
    ...
    return {
      ...
      total: wtIsAll
        ? matched.length
        : new Set(matched.map((v: any) => v.visitor_id)).size,
    };
  });
  ...
  const webTraffic = [
    {
      label: "All Visitors",
      tooltip: "Total unique visitors or page views across all pages",
      total: allVisitorsCount,
    },
    ...pageTraffic,
    {
      label: "Started Application",
      tooltip: "Began the nanny application form",
      total: startedApp,
    },
    {
      label: "Completed App",
      tooltip: "Submitted the nanny application",
      total: completedApp,
    },
    {
      label: "Nanny Accounts",
      tooltip: "Nanny accounts created",
      total: nannyAccounts,
    },
    {
      label: "Parent Accounts",
      tooltip: "Parent accounts created",
      total: parentAccounts,
    },
  ];
  ...
  const wtRefRecords = wtReferrers.map((ref) => {
    ...
    return {
      label: ref.label,
      tooltip: `Traffic from ${refName}`,
      records: wtVisits.filter(
        (v: any) => (v.referrer || "Direct") === refName,
      ),
      idKey: "visitor_id",
    };
  });
  const wtCompletedLeads = wtLeads.filter((l: any) =>
    ["applied", "ai_generated", "converted"].includes(l.lead_status),
  );
  ...
  internalCatalog.set("wt", [
    {
      label: "All Visitors",
      tooltip: "Total unique visitors or page views",
      records: wtVisits,
      idKey: "visitor_id",
    },
    ...wtPageRecords,
  ...
  const nfActiveConns = nfCohortConn.filter(
    (c: any) => !TERMINAL_STAGES.has(c.connection_stage),
  );
  internalCatalog.set("nf", [
    ...
    },
    {
      label: "Submitted Form",
      tooltip: "Completed and submitted the application",
      records: nfLeads.filter((l: any) =>
        ["applied", "ai_generated", "converted"].includes(l.lead_status),
      ),
      liveRecords: nfLeads.filter((l: any) => l.lead_status === "applied"),
      idKey: "id",
    },
    {
      label: "Profile Generated",
      ...
      liveRecords: nfLeads.filter((l: any) => l.lead_status === "ai_generated"),
      idKey: "id",
    },
    {
      label: "Account Created",
      tooltip: "Nanny account registered on the platform",
      records: nfLeads.filter((l: any) => l.lead_status === "converted"),
      idKey: "id",
    },
    {
      label: "Fully Verified",
      tooltip: "All verification steps completed (ID + WWCC + references)",
      records: nfNannies.filter((n: any) => n.verification_level === 4),
      idKey: "user_id",
    },
    {
      label: "Connections",
      tooltip:
        "Total connection requests involving this nanny (toggle unique/all)",
      records: nfCohortConn,
      liveRecords: nfActiveConns,
      idKey: "nanny_id",
    },
  ...
  if (pageDropoff) {
    internalCatalog.set(
      ...
      pageStages.map((s, i) => ({
        label: s.label,
        tooltip: s.tooltip,
        records: leadsWithPages.filter(
          (l: any) => l.highest_page_reached >= i + 1,
        ),
        liveRecords: leadsWithPages.filter(
          (l: any) => l.highest_page_reached === i + 1,
        ),
        idKey: "id",
      })),
  ...
  const pcParentsWithoutPos = pcParents.filter(
    (p: any) => !pcParentsWithPos.has(p.id),
  );
  const pcParentsWithPosNoConn = Array.from(pcParentsWithPos)
    .filter((id) => !new Set(pcConns.map((c: any) => c.parent_id)).has(id))
    .map((id) => ({ parent_id: id }));
  internalCatalog.set("pc", [
    {
      ...
      tooltip: "Total parent accounts in this cohort",
      records: pcParents,
      liveRecords: pcParentsWithoutPos,
      idKey: "user_id",
    },
    {
      label: "Positions",
  ...
  // Nanny Connections
  const ncNanniesNoConn = ncNannies.filter(
    (n: any) => !new Set(ncConns.map((c: any) => c.nanny_id)).has(n.id),
  );
  ...
  internalCatalog.set("nv", [
    {
      label: "Unverified",
      tooltip: "No verification completed (Level 0)",
      records: nvNannies.filter((n: any) => n.verification_level === 0),
      idKey: "user_id",
    },
    {
      label: "ID Verified",
      tooltip: "Identity document verified (Level 1)",
      records: nvNannies.filter((n: any) => n.verification_level >= 1),
      liveRecords: nvNannies.filter((n: any) => n.verification_level === 1),
      idKey: "user_id",
    },
    {
      label: "WWCC Verified",
      tooltip: "Working With Children Check verified (Level 2)",
      records: nvNannies.filter((n: any) => n.verification_level >= 2),
      liveRecords: nvNannies.filter((n: any) => n.verification_level === 2),
      idKey: "user_id",
    },
    {
      label: "Provisional",
      tooltip: "Basic verifications complete (Level 3)",
      records: nvNannies.filter((n: any) => n.verification_level >= 3),
      liveRecords: nvNannies.filter((n: any) => n.verification_level === 3),
      idKey: "user_id",
    },
    {
      label: "Fully Verified",
      tooltip: "All verification steps completed (Level 4)",
      records: nvNannies.filter((n: any) => n.verification_level >= 4),
      idKey: "user_id",
    },
  ]);

  // Identity Verification
  const niFailedTotal = niNannyVerifs.filter(
    (v: any) =>
      v.identity_status === "failed" || v.identity_status === "rejected",
  );
  internalCatalog.set("ni", [
    {
      label: "ID: Initiated",
      tooltip: "Started identity verification",
      records: niNannyVerifs.filter(
        (v: any) => v.identity_status !== "not_started",
      ),
      idKey: "user_id",
    },
    {
      label: "ID: Processing",
      tooltip: "Identity document being processed",
      records: niNannyVerifs.filter((v: any) =>
        IDENTITY_PAST_PENDING.includes(v.identity_status),
      ),
      idKey: "user_id",
    },
    ...
    {
      label: "ID: Unverified",
      tooltip: "Not yet started identity verification",
      records: niNannyVerifs.filter(
        (v: any) => v.identity_status === "not_started",
      ),
      idKey: "user_id",
    ...
    {
      label: "ID: Failed (Total)",
      tooltip: "All failed identity checks (auto + manual)",
      records: niFailedTotal,
      idKey: "user_id",
    },
    {
      label: "ID: Failed (Auto)",
      tooltip: "Automated identity check failed",
      records: niNannyVerifs.filter((v: any) => v.identity_status === "failed"),
      idKey: "user_id",
    },
    {
      label: "ID: Rejected",
      tooltip: "Manually rejected by admin review",
      records: niNannyVerifs.filter(
        (v: any) => v.identity_status === "rejected",
      ),
      idKey: "user_id",
    },
    {
      label: "ID: In Review",
      tooltip: "Flagged for manual admin review",
      records: niNannyVerifs.filter((v: any) => v.identity_status === "review"),
      idKey: "user_id",
    },
  ]);

  // WWCC Verification
  const nwNegativeStatuses = [
    "failed",
    "rejected",
    "barred",
    "ocg_not_found",
  ...
  internalCatalog.set("nw", [
    {
      label: "WWCC: Initiated",
      tooltip: "Started WWCC process",
      records: nwNannyVerifs.filter(
        (v: any) => v.wwcc_status !== "not_started",
      ),
      idKey: "user_id",
    },
    {
      ...
      tooltip: "WWCC application submitted",
      records: nwNannyVerifs.filter((v: any) =>
        WWCC_PAST_APP_PENDING.includes(v.wwcc_status),
      ),
      idKey: "user_id",
    },
    ...
    },
    {
      ...
      tooltip: "WWCC document verified and current",
      records: nwNannyVerifs.filter(
        (v: any) => v.wwcc_status === "doc_verified",
      ),
      idKey: "user_id",
    },
    {
      label: "WWCC: Unverified",
      tooltip: "Not yet started WWCC verification",
      records: nwNannyVerifs.filter(
        (v: any) => v.wwcc_status === "not_started",
      ),
      idKey: "user_id",
    ...
    {
      label: "WWCC: Failed (Total)",
      tooltip: "All negative WWCC outcomes combined",
      records: nwNannyVerifs.filter((v: any) =>
        nwNegativeStatuses.includes(v.wwcc_status),
      ),
      idKey: "user_id",
    },
    {
      label: "WWCC: Failed (Auto)",
      tooltip: "Automated WWCC check failed",
      records: nwNannyVerifs.filter((v: any) => v.wwcc_status === "failed"),
      idKey: "user_id",
    },
    {
      label: "WWCC: Rejected",
      tooltip: "WWCC manually rejected",
      records: nwNannyVerifs.filter((v: any) => v.wwcc_status === "rejected"),
      idKey: "user_id",
    },
    {
      label: "WWCC: OCG Not Found",
      tooltip: "Office of Children's Guardian record not found",
      records: nwNannyVerifs.filter(
        (v: any) => v.wwcc_status === "ocg_not_found",
      ),
      idKey: "user_id",
    },
    {
      label: "WWCC: Barred",
      tooltip: "Person barred from working with children",
      records: nwNannyVerifs.filter((v: any) => v.wwcc_status === "barred"),
      idKey: "user_id",
    },
    {
      label: "WWCC: Expired",
      tooltip: "WWCC has expired and needs renewal",
      records: nwNannyVerifs.filter((v: any) => v.wwcc_status === "expired"),
      idKey: "user_id",
    },
  ]);

  // Parent Verification (always include — totals will be 0 if no records)
  const pvFailedTotal = pvParentVerifs.filter(
    (v: any) =>
      v.identity_status === "failed" || v.identity_status === "rejected",
  );
  internalCatalog.set("pv", [
    ...
    },
    {
      label: "ID: Processing",
      tooltip: "Identity document being processed",
      records: pvParentVerifs.filter((v: any) =>
        IDENTITY_PAST_PENDING.includes(v.identity_status),
      ),
      idKey: "user_id",
    },
    ...
    },
    {
      ...
      tooltip: "Automated identity check failed",
      records: pvParentVerifs.filter(
        (v: any) => v.identity_status === "failed",
      ),
      idKey: "user_id",
    },
    {
  ...
  const dfFilterDate = (records: any[], dateField = "created_at") => {
    ...
    return records.filter((r: any) => {
      ...
      if (
        dfRange.from &&
        dfRange.from !== "1970-01-01" &&
        ts < `${dfRange.from}T00:00:00`
      )
        return false;
  ...
  const dfyInterestedKeys = new Set(
    dfyNotifications
      .filter((n: any) => n.status === "interested")
      .map((n: any) => `${n.nanny_id}|${n.position_id}`),
  );
  const dfConns = connections.filter(
    (c: any) =>
      c.position_id && dfyInterestedKeys.has(`${c.nanny_id}|${c.position_id}`),
  );
  ...
  const dfyMatchmaking = [
    {
      label: "DFY Activated",
      tooltip: "Positions with DFY matchmaking triggered",
      total: dfCnt(0, dfPositions, "id"),
      override: dfRo[0],
      tags: PT,
    },
    {
      label: "Standard Tier",
      tooltip: "DFY standard tier activations",
      total: dfCnt(
        1,
        dfPositions.filter((p: any) => p.dfy_tier === "standard"),
        "id",
      ),
      override: dfRo[1],
      tags: PT,
    ...
    {
      label: "Priority Tier",
      tooltip: "DFY priority tier activations",
      total: dfCnt(
        2,
        dfPositions.filter((p: any) => p.dfy_tier === "priority"),
        "id",
      ),
      override: dfRo[2],
      tags: PT,
    },
    {
      ...
      tooltip: "DFY searches still within expiry window",
      total: dfCnt(
        3,
        dfPositions.filter(
          (p: any) =>
            p.dfy_expires_at && new Date(p.dfy_expires_at).getTime() > now,
        ),
        "id",
      ),
      override: dfRo[3],
    ...
    {
      label: "Expired",
      tooltip: "DFY searches past expiry window",
      total: dfCnt(
        4,
        dfPositions.filter(
          (p: any) =>
    ...
    {
      label: "Nannies Notified",
      tooltip: "Unique nannies sent DFY notifications (all waves)",
      total: dfCnt(5, dfNotifsSent, "nanny_id"),
      override: dfRo[5],
      tags: NT,
    },
    {
      label: "Wave 1",
      tooltip: "Nannies notified in wave 1",
      total: dfCnt(
        6,
        dfNotifsSent.filter((n: any) => n.wave === 1),
        "nanny_id",
      ),
      override: dfRo[6],
      tags: NT,
    ...
    {
      label: "Wave 2",
      tooltip: "Nannies notified in wave 2",
      total: dfCnt(
        7,
        dfNotifsSent.filter((n: any) => n.wave === 2),
        "nanny_id",
      ),
      override: dfRo[7],
      tags: NT,
    ...
    {
      label: "Wave 3",
      tooltip: "Nannies notified in wave 3",
      total: dfCnt(
        8,
        dfNotifsSent.filter((n: any) => n.wave === 3),
        "nanny_id",
      ),
      override: dfRo[8],
      tags: NT,
    ...
    {
      label: "Viewed",
      tooltip: "Nannies who viewed the DFY notification",
      total: dfCnt(
        9,
        dfNotifs.filter((n: any) => n.viewed_at),
        "nanny_id",
      ),
      override: dfRo[9],
      tags: NT,
    ...
    {
      label: "Interested",
      tooltip: "Nannies who responded with interest",
      total: dfCnt(
        10,
        dfNotifs.filter((n: any) => n.status === "interested"),
        "nanny_id",
      ),
      override: dfRo[10],
      tags: NT,
    ...
    {
      label: "No Response",
      tooltip: "Notifications that expired without response",
      total: dfCnt(
        11,
        dfNotifs.filter((n: any) => n.status === "expired"),
        "nanny_id",
      ),
      override: dfRo[11],
      tags: NT,
  ...
  const dfyTimestamps: (number | null)[][] = dfNotifsSent.map((n: any) => [
    ...
    null,
    null,
    null, // position-level stages (no per-notification timestamp)
  ...
  const dfyMatchmakingLive = [
    ...
    {
      label: "Standard Tier",
      total: dfPositions.filter((p: any) => p.dfy_tier === "standard").length,
      tags: PT,
    },
    {
      label: "Priority Tier",
      total: dfPositions.filter((p: any) => p.dfy_tier === "priority").length,
      tags: PT,
    },
    {
      label: "Active (Not Expired)",
      total: dfPositions.filter(
        (p: any) =>
          p.dfy_expires_at && new Date(p.dfy_expires_at).getTime() > now,
      ).length,
      tags: PT,
    ...
    {
      label: "Nannies Notified",
      total: dfNotifs.filter((n: any) => n.status === "notified").length,
      tags: NT,
    },
    {
      label: "Wave 1",
      total: dfNotifs.filter(
        (n: any) => n.wave === 1 && n.status === "notified",
      ).length,
      tags: NT,
    },
    {
      label: "Wave 2",
      total: dfNotifs.filter(
        (n: any) => n.wave === 2 && n.status === "notified",
      ).length,
      tags: NT,
    },
    {
      label: "Wave 3",
      total: dfNotifs.filter(
        (n: any) => n.wave === 3 && n.status === "notified",
      ).length,
      tags: NT,
    },
    {
      label: "Viewed",
      total: dfNotifs.filter((n: any) => n.status === "viewed").length,
      tags: NT,
    },
    {
      label: "Interested",
      total: dfNotifs.filter((n: any) => n.status === "interested").length,
      tags: NT,
    },
    {
      label: "No Response",
      total: dfNotifs.filter((n: any) => n.status === "expired").length,
      tags: NT,
    },
  ];
  ...
  internalCatalog.set("df", [
    ...
    },
    {
      label: "Standard Tier",
      tooltip: "DFY standard tier activations",
      records: dfPositions.filter((p: any) => p.dfy_tier === "standard"),
      idKey: "id",
      tags: PT,
    },
    {
      label: "Priority Tier",
      tooltip: "DFY priority tier activations",
      records: dfPositions.filter((p: any) => p.dfy_tier === "priority"),
      idKey: "id",
      tags: PT,
    },
    {
    ...
    {
      label: "Nannies Notified",
      tooltip: "Unique nannies sent DFY notifications",
      records: dfNotifsSent,
      liveRecords: dfNotifs.filter((n: any) => n.status === "notified"),
      idKey: "nanny_id",
      tags: NT,
    },
    {
      label: "Wave 1",
      tooltip: "Nannies notified in wave 1",
      records: dfNotifsSent.filter((n: any) => n.wave === 1),
      idKey: "nanny_id",
      tags: NT,
    },
    {
      label: "Wave 2",
      tooltip: "Nannies notified in wave 2",
      records: dfNotifsSent.filter((n: any) => n.wave === 2),
      idKey: "nanny_id",
      tags: NT,
    },
    {
      label: "Wave 3",
      tooltip: "Nannies notified in wave 3",
      records: dfNotifsSent.filter((n: any) => n.wave === 3),
      idKey: "nanny_id",
      tags: NT,
    },
    {
      label: "Viewed",
      tooltip: "Nannies who viewed the notification",
      records: dfNotifs.filter((n: any) => n.viewed_at),
      liveRecords: dfNotifs.filter((n: any) => n.status === "viewed"),
      idKey: "nanny_id",
      tags: NT,
    },
    {
      label: "Interested",
      tooltip: "Nannies who responded with interest",
      records: dfNotifs.filter((n: any) => n.status === "interested"),
      idKey: "nanny_id",
      tags: NT,
    },
    {
      label: "No Response",
      tooltip: "Notifications expired without response",
      records: dfNotifs.filter((n: any) => n.status === "expired"),
      idKey: "nanny_id",
      tags: NT,
    },
  ]);
  ...
  const dfyConnectionsCumulative = [
    {
      label: "All DFY Connections",
      tooltip: "Total connections sourced from DFY matchmaking",
      total: countWithOverride(dcConns, "nanny_id", dcCfg.count, dcRo[0]),
      override: dcRo[0],
    },
  ...
  const dfyConnectionsLive = [
    {
      ...
      total:
        ...
        new Set(
          dcConns
            .filter((c: any) => !TERMINAL_STAGES.has(c.connection_stage))
            .map((c: any) => c.nanny_id),
        ).size,
      override: dcRo[0],
    },
  ...
  const activeConnsAll = connections.filter(
    (c: any) => !TERMINAL_STAGES.has(c.connection_stage),
  );
  ...
  internalCatalog.set("kn", [
    ...
    },
    {
      ...
      tooltip: "Verified for matchmaking (Level 2+)",
      records: activeNannies,
      idKey: "user_id",
    },
    {
      label: "Placements",
      tooltip: "Nannies with a confirmed placement",
      records: g.placements,
      idKey: "nanny_id",
    },
  ]);
  ...
  internalCatalog.set("kp", [
    {
      label: "Parents",
      tooltip: "Total parent accounts",
      records: g.parents,
      idKey: "user_id",
    },
    {
      label: "Positions",
      tooltip: "Total position listings",
      records: g.positions,
      idKey: "id",
    },
    {
      label: "Connections",
      tooltip: "Total connection requests",
      records: connections,
      liveRecords: activeConnsAll,
      idKey: "id",
    },
    {
      label: "Placements",
      tooltip: "Parents with a confirmed placement",
      records: g.placements,
      idKey: "parent_id",
    },
  ]);
      ...
      return {
        ...
        stages: visible.map((s) => ({
          ...
          total: countWithOverride(
            s.records,
            s.idKey,
            (configs[key] || { count: "unique" as const }).count,
          ),
          ...(s.tags ? { tags: s.tags } : {}),
        })),
      };
  ...
  return {
    ...
    parentTimestamps,
    parentFunnelLive,
    parentSourceBreakdown,
    parentConnCumulative,
    parentConnLive,
    parentConnTimestamps,
    nannyConnCumulative,
    nannyConnLive,
    nannyConnTimestamps,
    parentTerminalStages,
    nannyTerminalStages,
    avgConnsPerPos,
    fillRate,
    nannyVerifLevelsCumulative,
    nannyVerifLevelsLive,
    nannyIdentityCumulative,
    nannyIdentityLive,
    nannyWwccCumulative,
    nannyWwccLive,
    parentIdentityCumulative,
    parentIdentityLive,
    dfyMatchmaking,
    dfyMatchmakingLive,
    dfyTimestamps,
    dfyConnectionsCumulative,
    dfyConnectionsLive,
    catalog,
    customStages,
  };
            ...
            <CardContent className="py-3 px-5">
              <div className="flex flex-wrap items-center gap-x-6 gap-y-2 text-[13px]">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="font-bold text-slate-900 tabular-nums">
                    {data.totalNannySignups}
                  </span>
                </div>
                <div className="flex items-center gap-1.5">
                  <span className="text-slate-400">Parents</span>
                  <span className="font-bold text-slate-900 tabular-nums">
                    {data.totalParentSignups}
                  </span>
                </div>
                <div className="flex items-center gap-1.5">
                ...
                <div className="flex items-center gap-1.5">
                  ...
                  <span className="font-bold text-slate-900 tabular-nums">
                    {data.supplyCount}:{data.demandCount}
                  </span>
                </div>
                <div className="flex items-center gap-1.5">
          ...
          <Card>
            <CardContent className="pt-5">
              <PipelineTable
                ...
                timestamps={
                  data.dfyTimestamps.length > 0 ? data.dfyTimestamps : undefined
                }
                tableKey="df"
              />
            </CardContent>
