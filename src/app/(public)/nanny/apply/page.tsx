  ...
  if (user) {
    ...
    if (role?.role === "parent") {
      ...
      const { data: nanny } = await admin
        .from("nannies")
        .select(
          "visible_in_bsr, identity_verified, wwcc_verified, total_experience_years, hourly_rate_min",
        )
        .eq("user_id", user.id)
        .maybeSingle();
  ...
  const steps = [
    {
      ...
      description:
        "Sign up as a childcare professional and tell us about yourself.",
    ...
    },
    {
      ...
      description:
        "Add your experience, qualifications, availability, and rate.",
    ...
    },
    {
      ...
      description:
        "Once verified, you can receive and apply for childcare opportunities in your area.",
      ...
      <div className="mx-auto max-w-sm px-4 py-8 space-y-6">
        <div className="text-center">
          ...
          <p className="text-sm text-slate-500 mt-1">
            To receive private childcare opportunities
            <br />
        ...
        {userState === "parent" && (
          <div className="rounded-lg bg-amber-50 border border-amber-200 p-4 text-sm text-amber-800">
            You are currently signed in with a parent account. To apply as a
            babysitter, you need to create a separate childcare professional
            account.
          </div>
        )}
          ...
          {steps.map((step, i) => {
            ...
            return (
              <div key={i} className="relative flex items-start gap-3 py-3">
                {/* Step indicator */}
                <div
                  className={`relative z-10 flex items-center justify-center h-10 w-10 rounded-full border-2 flex-shrink-0 ${
                    step.done
        ...
        {userState === "nanny_complete" && (
          <div className="rounded-lg bg-green-50 border border-green-200 p-4 text-sm text-green-800 text-center">
            You&apos;re all set! You can now receive and apply for childcare
            opportunities.
          </div>
        )}
      </div>
