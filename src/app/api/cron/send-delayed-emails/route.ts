...
<div style="max-width:600px;margin:0 auto;padding:32px 16px;">
  ...
  for (const conn of awaitingConnections ?? []) {
    ...
    const nannyInfo = nannyData
      ? await getUserEmailInfo(nannyData.user_id)
      : null;
...
<div style="max-width:600px;margin:0 auto;padding:32px 16px;">
  ...
  for (const conn of staleConnections ?? []) {
    ...
    const parentInfo = parentData
      ? await getUserEmailInfo(parentData.user_id)
      : null;
...
<div style="max-width:600px;margin:0 auto;padding:32px 16px;">
  ...
  for (const v of stuckNannies ?? []) {
    ...
    await sendEmail({
      ...
      subject,
      ...(isLcy104
        ? { from: "Baby Bloom <hello@babybloomsydney.com.au>" }
        : {}),
...
function getLcyContent(
  ...
  const groupMessages: Record<string, string> = {
    // Group 1
    id_not_started:
      "To connect you with families, we need to verify your identity. It\u2019s a quick process \u2014 confirm your address, upload your passport, and take a selfie. Most people finish in under 3 minutes.",
    // Group 2
    wwcc_not_started:
      "Your identity is verified \u2014 you\u2019re one step away! Upload your Working With Children Check and your profile will be visible to families in your area.",
    // Group 3 sub-variants
    identity_failed:
      "Your identity check needs a quick fix \u2014 most people get it right on the second try. Head to your verification page to see what needs adjusting.",
    wwcc_failed:
      "Your WWCC check needs a quick fix. If one upload method didn\u2019t work, try another \u2014 the Grant Email PDF is usually the most reliable.",
    wwcc_ocg_not_found:
      "The Office of the Children\u2019s Guardian couldn\u2019t find your WWCC with the details provided. This usually means a typo in the WWCC number or a surname mismatch.",
    wwcc_expired:
      "Your WWCC needs to be renewed before we can verify you. Once you have a new clearance from the Office of the Children\u2019s Guardian, come back and resubmit.",
  };
  ...
  const instructions =
    groupInstructions[subVariant] || groupInstructions.id_not_started;
...
function buildLcyHtml(
  ...
  const paragraphs = bodyParagraphs
    .map(
      (p) =>
        `    <p style="font-size:15px;color:#475569;line-height:1.6;margin:0 0 12px;">${p}</p>`,
    )
    .join("\n");
