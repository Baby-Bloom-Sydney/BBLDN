...
import {
  ...
  updateAdminPosition,
  closeAdminPosition,
} from "@/lib/actions/admin-positions";
...
export async function GET(
  ...
  if (result.error || !result.position) {
    return NextResponse.json(
      { success: false, error: result.error ?? "Position not found" },
      { status: 404 },
    );
  }
...
export async function PATCH(
  ...
  const result = await updateAdminPosition(
    id,
    body as Parameters<typeof updateAdminPosition>[1],
  );
