...
// `CRON_SECRET`, and holding it does not let anyone forge a cron bearer. The only real coupling is rotation —
// rotating `CRON_SECRET` invalidates every visitor cookie, whose entire consequence is that a fresh id is
// minted on the next request. Recorded in L-009 PROGRESS with the alternative (a dedicated
// `VISITOR_COOKIE_SECRET`) for the planner to rule on.
...
export function visitorIdOf(request: Request): string | null {
  ...
  const parsed = SIGNED.exec(decodeURIComponent(raw));
