...
function funnelStateToProfile(state: NannyLeadFunnelState): PublicNannyProfile {
  ...
  return {
    ...
    nanny_experience_years:
      state.experience.childcare_roles
    ...
    min_child_age_months: state.preferences.min_age
      ? (AGE_TO_MONTHS[state.preferences.min_age] ?? null)
      : null,
    max_child_age_months: state.preferences.max_age
      ? (AGE_TO_MONTHS[state.preferences.max_age] ?? null)
      : null,
    ...
    strengths_traits:
      state.identity.personality_traits.length > 0
    ...
    childcare_roles:
      state.experience?.childcare_roles?.map(
        (r: { role: string; duration: number }) => ({
          role: r.role,
          duration: r.duration || 0,
        }),
      ) || null,
    additional_photos: [
      state.about_you?.photo_1_url,
      state.about_you?.photo_2_url,
      state.about_you?.photo_3_url,
    ...
    <div className="relative min-h-screen pb-24">
      ...
      <div className="max-w-2xl mx-auto px-4 pt-8 pb-2 text-center animate-in fade-in duration-500">
        ...
        <p className="mt-2 text-sm text-slate-500 leading-relaxed max-w-sm mx-auto">
          We&apos;ve built your professional childcare portfolio, ready to
          connect you with our network of families!
        </p>
      </div>

      {/* ── Profile (rises up) ── */}
      <div
        className="animate-in slide-in-from-bottom-6 fade-in duration-700 fill-mode-both px-4"
        style={{ animationDelay: "400ms" }}
      >
        <ParentNannyProfileView nanny={profile} isActiveNanny />
      </div>
