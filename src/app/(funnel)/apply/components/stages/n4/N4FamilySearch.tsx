...
export function N4FamilySearch({ state, dispatch, goNext }: StageProps) {
    ...
    async function runWithRetry(leadId: string, attempts = 2) {
      for (let i = 0; i < attempts; i++) {
        try {
          ...
          console.error(
            `[N4] AI generation attempt ${i + 1} failed:`,
            result.error,
          );
        } catch (err) {
  ...
  const searchItems = useMemo(() => {
    ...
    if (availability.available_days.length > 0) {
      const dayStr =
        availability.available_days.length >= 5
          ? "Weekday positions..."
    ...
    return (
      <div className="flex flex-col items-center gap-6 py-16 animate-in fade-in zoom-in-95 duration-500">
        ...
        <div className="text-center max-w-sm">
          <h2 className="text-2xl font-bold text-slate-800">Good news!</h2>
          <p className="mt-3 text-sm text-slate-600 leading-relaxed">
            There are multiple families looking for a nanny like you that match
