...
import { MapPin, ShieldCheck, GraduationCap, Check } from "lucide-react";
...
export function PublicMatchCard({ match }: PublicMatchCardProps) {
  ...
  if (nanny.nanny_experience_years && nanny.nanny_experience_years > 0)
    traitBadges.push({
      icon: "Clock",
      label: `${nanny.nanny_experience_years}${nanny.nanny_experience_years === 1 ? "yr" : "yrs"} experience`,
      primary: true,
    });
  ...
  if (
    ...
    match.highestQualification.toLowerCase() !== "no qualification"
  ) {
  ...
  return (
    <div className="relative h-full rounded-2xl border border-slate-200 bg-white shadow-sm overflow-hidden flex flex-col @container">
      <span
        className={`absolute top-3 right-3 z-10 inline-flex items-center gap-1.5 rounded-full px-3 py-1 text-xs font-semibold shrink-0 border ${getScoreBadgeStyle(match.finalScore)}`}
      >
      ...
      <div className="relative px-5 pb-5 flex-1 flex flex-col">
              ...
              <div className="min-w-0 pr-2">
                ...
                {qualLabel && (
                  <p className="flex items-center gap-1 text-sm text-slate-500 mt-0.5">
                    ...
                    <span className="truncate">{qualLabel}</span>
                  </p>
                )}
              </div>
            </div>
        ...
        {(match.overQualifiedBonuses.length > 0 ||
          match.unmetRequirements.length > 0) && (
          <div className="mt-3 pt-3 border-t border-slate-100 flex-1 overflow-hidden text-[clamp(8px,3cqw,13px)]">
            <div
              className="columns-2 gap-x-2 h-full max-h-[9lh]"
              style={{ columnFill: "auto" }}
            >
              {match.overQualifiedBonuses.map((b) => (
                <p
                  key={b}
                  className="text-green-600 leading-tight truncate break-inside-avoid"
                >
                  + {b}
                </p>
              ))}
              {match.unmetRequirements.map((r) => (
                <p
                  key={r}
                  className="text-amber-600 leading-tight truncate break-inside-avoid"
                >
                  - {r}
                </p>
              ))}
