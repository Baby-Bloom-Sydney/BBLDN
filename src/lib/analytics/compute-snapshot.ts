...
function snap(stages: IntStage[], defaultTags?: string[]): SnapshotStage[] {
  return stages.map((s) => ({
    ...
    live_unique: s.liveRecords
      ? new Set(s.liveRecords.map((r) => r[s.idKey])).size
      : null,
...
export async function computeSnapshot(): Promise<SectionSnapshot[]> {
  ...
  const [
    ...
    admin
      .from("verifications")
      .select("user_id, identity_status, wwcc_status, created_at"),
    admin
    ...
    admin
      .from("page_visits")
      .select("visitor_id, referrer_source, page_path, created_at"),
    admin
      .from("page_visits")
  ...
  // ── Apply filters ──
  const connections = ((connectionsRes.data || []) as any[]).filter(
    (c: any) =>
      !testNannyIds.has(c.nanny_id) && !testParentIds.has(c.parent_id),
  );
  const userStats = rawUS.filter((us: any) => !testUserIds.has(us.user_id));
  const verifs = ((verificationsRes.data || []) as any[]).filter(
    (v: any) => !testUserIds.has(v.user_id),
  );
  const dfyNotifs = ((dfyNotificationsRes.data || []) as any[]).filter(
    (n: any) => !testNannyIds.has(n.nanny_id) && !testPosIds.has(n.position_id),
  );
  ...
  const placements = ((placementsRes.data || []) as any[]).filter(
    (pl: any) =>
      !testNannyIds.has(pl.nanny_id) && !testParentIds.has(pl.parent_id),
  );
  const positions = ((positionsRes.data || []) as any[]).filter(
    (p: any) => !testParentIds.has(p.parent_id),
  );
  const leads = ((leadsRes.data || []) as any[]).filter(
    (l: any) => !testLeadIds.has(l.id),
  );
  ...
  // STATS (KPIs)
  // ═══════════════════════════════════════════════
  const uniqueVisitors = !visitsRes.error
    ? new Set(visits.map((v: any) => v.visitor_id)).size
    : 0;
  const supplyCount = nannies.filter(
    (n: any) => n.verification_level >= 2,
  ).length;
  const demandCount = positions.filter(
    (p: any) => p.status === "active" || p.status === "open",
  ).length;

  const parentCreateMap = new Map(
    parents.map((p: any) => [p.id, new Date(p.created_at).getTime()]),
  );
  ...
  for (const c of connections) {
    if (c.position_id)
      posConnMap.set(c.position_id, (posConnMap.get(c.position_id) || 0) + 1);
  }
  const pcc = Array.from(posConnMap.values());
  const avgConnsPerPos =
    pcc.length > 0
      ? Math.round((pcc.reduce((a, b) => a + b, 0) / pcc.length) * 10) / 10
      : null;

  const placedPosIds = new Set<string>();
  for (const c of connections) {
    if (c.position_id && PLACED_STAGES.has(c.connection_stage))
      placedPosIds.add(c.position_id);
  }
  const fillRate =
    positions.length > 0
      ? Math.round((placedPosIds.size / positions.length) * 100)
      : null;

  const kpi = (label: string, value: number | null): SnapshotStage => ({
    label,
    total_unique: value ?? 0,
    total_all: value ?? 0,
    live_unique: null,
    live_all: null,
  ...
  const wtStages: IntStage[] = [
    ...
    ...pageCats.map(({ label, match }) => ({
      label,
      records: visits.filter((v: any) => match(v.page_path)),
      idKey: "visitor_id",
    })),
  ...
  const nfStages: IntStage[] = [
    ...
    },
    {
      ...
      records: leads.filter((l: any) =>
        ["applied", "ai_generated", "converted"].includes(l.lead_status),
      ),
      liveRecords: leads.filter((l: any) => l.lead_status === "applied"),
      idKey: "id",
      tags: ["N"],
    },
    {
      ...
      liveRecords: leads.filter((l: any) => l.lead_status === "ai_generated"),
      idKey: "id",
      tags: ["N"],
    },
    {
      label: "Account Created",
      records: leads.filter((l: any) => l.lead_status === "converted"),
      idKey: "id",
      tags: ["N"],
    },
    {
      label: "Fully Verified",
      records: nannies.filter((n: any) => n.verification_level === 4),
      idKey: "user_id",
      tags: ["N"],
    },
    {
      label: "Requested",
      records: nfConns,
      liveRecords: nfActive.filter((c: any) => c.connection_stage === 0),
      idKey: "nanny_id",
      tags: ["N"],
    },
    {
      label: "Accepted",
      records: nfConns.filter((c: any) => c.connection_stage >= 10),
      liveRecords: nfActive.filter(
        (c: any) => c.connection_stage >= 10 && c.connection_stage <= 19,
      ),
      idKey: "nanny_id",
      tags: ["N"],
    },
    {
      ...
      records: nfConns.filter((c: any) => c.connection_stage >= 20),
      liveRecords: nfActive.filter((c: any) => c.connection_stage === 20),
      idKey: "nanny_id",
      tags: ["N"],
    },
    {
      label: "Meet Completed",
      records: nfConns.filter((c: any) => c.connection_stage >= 21),
      liveRecords: nfActive.filter(
        (c: any) => c.connection_stage >= 21 && c.connection_stage <= 30,
      ),
      idKey: "nanny_id",
      tags: ["N"],
    },
    {
      ...
      records: nfConns.filter((c: any) => c.connection_stage >= 31),
      liveRecords: nfActive.filter((c: any) => c.connection_stage === 31),
      idKey: "nanny_id",
      tags: ["N"],
    },
    {
      label: "Trial Completed",
      records: nfConns.filter((c: any) => c.connection_stage >= 32),
      liveRecords: nfActive.filter((c: any) => c.connection_stage === 32),
      idKey: "nanny_id",
      tags: ["N"],
    },
    {
      label: "Offered",
      records: nfConns.filter((c: any) => c.connection_stage >= 33),
      liveRecords: nfActive.filter((c: any) => c.connection_stage === 33),
      idKey: "nanny_id",
      tags: ["N"],
    },
    {
      label: "Placements",
      records: nfConns.filter((c: any) =>
        PLACED_STAGES.has(c.connection_stage),
      ),
      liveRecords: nfConns.filter((c: any) => c.connection_stage === 34),
      idKey: "nanny_id",
      tags: ["N"],
    },
  ];

  const nfUS = userStats.filter(
    (us: any) => us.user_type === "nanny" && nannyUserIdSet.has(us.user_id),
  );
  const nfTS: (number | null)[][] = nfUS.map((us: any) => [
    null,
    toMs(us.lead_created_at),
    ["applied", "ai_generated", "converted"].includes(us.lead_status)
      ? toMs(us.lead_created_at)
      : null,
    ["ai_generated", "converted"].includes(us.lead_status)
      ? toMs(us.lead_created_at)
      : null,
    us.lead_status === "converted" ? toMs(us.account_created_at) : null,
    null,
    toMs(us.first_connection_at),
    toMs(us.first_accepted_at),
    null,
    toMs(us.first_meetup_at),
    null,
    null,
    null,
    toMs(us.first_placement_at),
  ]);
  ...
  const pfUS = userStats.filter(
    (us: any) => us.user_type === "parent" && parentUserIdSet.has(us.user_id),
  );
  const pfTS: (number | null)[][] = pfUS.map((us: any) => [
    toMs(us.account_created_at),
    toMs(us.first_position_at),
    toMs(us.first_connection_at),
    toMs(us.first_accepted_at),
    null,
    toMs(us.first_meetup_at),
  ...
  const pcStages: IntStage[] = [
    {
      label: "Parents",
      records: parents,
      liveRecords: parents.filter((p: any) => !pcPosSet.has(p.id)),
      idKey: "user_id",
      tags: ["P"],
    },
    {
  ...
  const nvStages: IntStage[] = [
    {
      label: "Unverified",
      records: nannies.filter((n: any) => n.verification_level === 0),
      idKey: "user_id",
    },
    {
      label: "ID Verified",
      records: nannies.filter((n: any) => n.verification_level >= 1),
      liveRecords: nannies.filter((n: any) => n.verification_level === 1),
      idKey: "user_id",
    },
    {
      label: "WWCC Verified",
      records: nannies.filter((n: any) => n.verification_level >= 2),
      liveRecords: nannies.filter((n: any) => n.verification_level === 2),
      idKey: "user_id",
    },
    {
      label: "Provisional",
      records: nannies.filter((n: any) => n.verification_level >= 3),
      liveRecords: nannies.filter((n: any) => n.verification_level === 3),
      idKey: "user_id",
    },
    {
      label: "Fully Verified",
  ...
  const niStages: IntStage[] = [
    ...
    {
      label: "Initiated",
      records: niVerifs.filter((v: any) => v.identity_status !== "not_started"),
      idKey: "user_id",
    },
    {
      label: "Processing",
      records: niVerifs.filter((v: any) =>
    ...
    {
      label: "Verified",
      records: niVerifs.filter((v: any) => v.identity_status === "verified"),
      idKey: "user_id",
    },
  ];
  ...
  const nwStages: IntStage[] = [
    ...
    {
      label: "Initiated",
      records: niVerifs.filter((v: any) => v.wwcc_status !== "not_started"),
      idKey: "user_id",
    },
    {
      label: "Submitted",
    ...
    {
      label: "Verified",
      records: niVerifs.filter((v: any) => v.wwcc_status === "doc_verified"),
      idKey: "user_id",
    },
  ];
  ...
  const pvStages: IntStage[] = [
    ...
    {
      label: "Initiated",
      records: pvVerifs.filter((v: any) => v.identity_status !== "not_started"),
      idKey: "user_id",
    },
    {
      label: "Processing",
      records: pvVerifs.filter((v: any) =>
        IDENTITY_PAST_PENDING.includes(v.identity_status),
      ),
      idKey: "user_id",
    },
    ...
    {
      label: "Verified",
      records: pvVerifs.filter((v: any) => v.identity_status === "verified"),
      idKey: "user_id",
    },
  ];
  ...
  const dfStages: IntStage[] = [
    ...
    {
      label: "Standard Tier",
      records: dfyPos.filter((p: any) => p.dfy_tier === "standard"),
      idKey: "id",
      tags: PT,
    },
    {
      label: "Priority Tier",
      records: dfyPos.filter((p: any) => p.dfy_tier === "priority"),
      idKey: "id",
      tags: PT,
    },
    {
      label: "Active (Not Expired)",
      records: dfyPos.filter(
        (p: any) =>
    ...
    },
    {
      ...
      records: dfSent,
      liveRecords: dfyNotifs.filter((n: any) => n.status === "notified"),
      idKey: "nanny_id",
      tags: NT,
    },
    {
      label: "Wave 1",
      records: dfSent.filter((n: any) => n.wave === 1),
      idKey: "nanny_id",
      tags: NT,
    },
    {
      label: "Wave 2",
      records: dfSent.filter((n: any) => n.wave === 2),
      idKey: "nanny_id",
      tags: NT,
    },
    {
      label: "Wave 3",
      records: dfSent.filter((n: any) => n.wave === 3),
      idKey: "nanny_id",
      tags: NT,
    },
    {
      label: "Viewed",
      records: dfyNotifs.filter((n: any) => n.viewed_at),
      liveRecords: dfyNotifs.filter((n: any) => n.status === "viewed"),
      idKey: "nanny_id",
      tags: NT,
    },
    {
      label: "Interested",
      records: dfyNotifs.filter((n: any) => n.status === "interested"),
      idKey: "nanny_id",
      tags: NT,
    },
    {
      label: "No Response",
      records: dfyNotifs.filter((n: any) => n.status === "expired"),
      idKey: "nanny_id",
      tags: NT,
    },
  ];
  ...
  // ═══════════════════════════════════════════════
  const dfyIntKeys = new Set(
    dfyNotifs
      .filter((n: any) => n.status === "interested")
      .map((n: any) => `${n.nanny_id}|${n.position_id}`),
  );
  const dcConns = connections.filter(
    (c: any) =>
      c.position_id && dfyIntKeys.has(`${c.nanny_id}|${c.position_id}`),
  );
  const dcActive = dcConns.filter(
    (c: any) => !TERMINAL_STAGES.has(c.connection_stage),
  );
  ...
  const unknownParents = parents.filter(
    (p: any) => !p.signup_source || !knownSrcValues.has(p.signup_source),
  );
  psStages.push({
    label: "Unknown",
    records: unknownParents,
    idKey: "user_id",
    tags: ["P"],
  });
  ...
  const tpStages: IntStage[] = Object.entries(TERMINAL_LABELS).map(
    ([stage, label]) => ({
      label,
      records: connections.filter(
        (c: any) => c.connection_stage === parseInt(stage),
      ),
      idKey: "parent_id",
      tags: ["P"] as string[],
    }),
  ...
  // ═══════════════════════════════════════════════
  const tnStages: IntStage[] = Object.entries(TERMINAL_LABELS).map(
    ([stage, label]) => ({
      label,
      records: connections.filter(
        (c: any) => c.connection_stage === parseInt(stage),
  ...
  const allActiveConns = connections.filter(
    (c: any) => !TERMINAL_STAGES.has(c.connection_stage),
  );
