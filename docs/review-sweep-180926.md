...
- **Authority is session-derived on all 21 server actions in the diff** (16 new, 5 changed). No surface takes a role, id or ownership claim from form input and acts on it; `parseForm` whitelists per action, so no extra field can smuggle one.
