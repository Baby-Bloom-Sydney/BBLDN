...
### H-10 — **CLOSED by HARDEN-B (ADR-143)** — an absent booker is reconstructed as a branded empty string
...
### H-12 — **CLOSED by HARDEN-B** — P-7 never cascades K-24, and `connections` already has the arm for it
...
Outcome: an admin closes a position and every live connection on it stays in a live stage; both parties keep a connection pointing at a closed position, and `liveCountForPosition` keeps counting them. Two halves of one seam, built by two units, never joined. Was pinned `it.fails` at `positions.inside.test.ts:333`. **Closed by HARDEN-B:** `positions` now dispatches K-24 on every connection in `CANCELLABLE_STAGES`, through `advance` on the caller's `uow`, with the `cascade` job as actor so `fromPositionClose` reads true; a close that cannot read its connection list refuses.

### H-13 — **CLOSED by HARDEN-B (ADR-144)** — `payments.prices()` answers `[]` when unconfigured
...
**Ruled by ADR-144** — the second option: `prices()` keeps its signature and the page treats `[]` as `E_PROVIDER`. `selfServeShapes()` logs `ALERT_PROVIDER_DOWN` and `SelfServePage` renders the payments-unavailable state. Closed by HARDEN-B.
...
| M-13 | **CLOSED (HARDEN-B).** `settle` discards the update result; settling a non-existent message succeeds                                                | `boot/db-comms-store.ts:115-124`                                                                                       | A patch matching zero rows is indistinguishable from one that matched, so a settle against an unknown `messageId` reports `ok`.                                                                                                                                                                                                                                                                                                                                                  | `1h`                               |
...
| M-15 | **CLOSED (HARDEN-B).** An auth-read failure on the parent dashboard is indistinguishable from signed-out                                            | `src/app/parent/page.tsx:52-58`                                                                                        | A failed `getCurrentUserId` yields `familyId === null`, which skips the gate entirely and renders the logged-out page, with no log. The gate itself below it is handled correctly and commented; the read above it is not.                                                                                                                                                                                                                                                       | route / `1i`                       |
| M-16 | **CLOSED (HARDEN-B).** `default:` on a discriminated union re-narrows the discriminant with a lying cast                                            | `matching/components/wizard/QuestionBody.tsx:80-82`                                                                    | Adding a sixth `kind` compiles silently and renders `ChoiceChips` with `options ?? []` — a blank, unanswerable question. A `const _exhaustive: never = question` would have made the addition a compile error.                                                                                                                                                                                                                                                                   | `1e`                               |
...
**Eleven at the sweep** (ten inherited, one added by it). **Six after HARDEN-B and S5d:** rows 4, 8, 10 and 11 below are closed by real behaviour, S5d closed one of 5–7, and HARDEN-B added one new pin (ADR-145 (2)'s captured email, against `0014`). The counts below are the sweep's own. `vitest run` reports **3,540 passed | 11 expected fail** — so none has quietly turned true in the vitest sense; a pin whose body stopped throwing would report as a _failure_, not a pass.
...
| 4 (closed)  | `call-layer/__tests__/call-layer.inside.test.ts:736`   | 03 §3.5 seq 3 parent-over-nanny displacement                                                                                             | **Honest in what it pins, STALE in why.** Displacement _is_ built (`scheduling-booking-writes.ts:117-156` computes `displaceTo` and passes `p_displace_to`). The pin is red only because the test wires `createSchedulingStub`, so its own promise — "the day it is built this `it.fails` turns red" — is already false. It now pins test scaffolding, not a doc/code disagreement. **Closed by HARDEN-B:** the stub now implements I-2 / I-3 / I-13 with the same pure `nextFreeSlot` the inside uses, and the test asserts the move. |
...
| 8 (closed)  | `scheduling/__tests__/scheduling.inside.test.ts:623`   | 03 §3.2: a position booked on behalf reads back carrying its parent                                                                      | **Honest.** `bookings` stores `subject_type` + `subject_id` only.                                                                                                                                                                                                                                                                                                                                                                                                                                                                      |
...
| 10 (closed) | `positions/__tests__/positions.inside.test.ts:333`     | 03 §2.4 P-7 cascades K-24 on every live connection                                                                                       | **Honest in what it pins, STALE in why.** The gap is real and shipped (H-12); the stated owner (`1g`) has landed and built the receiving half.                                                                                                                                                                                                                                                                                                                                                                                         |
| 11 (closed) | `scheduling/__tests__/scheduling.inside.test.ts` (new) | 03 §3.2: an absent booker is never a branded empty id                                                                                    | **Added by this sweep** — H-10.                                                                                                                                                                                                                                                                                                                                                                                                                                                                                                        |
