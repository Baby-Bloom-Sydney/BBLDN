...
const SURFACES = ["src/modules", "src/app/(public)", "src/app/(funnel)", "src/app/(auth)", "src/app/parent"];
...
function isSurfaceFile(file) {
  return !EXCLUDED_SEGMENTS.some((segment) => file.includes(segment))
...
function findHits(file, pattern) {
  return readFileSync(file, "utf8").split("\n").flatMap((line, index) => {
    ...
    return matches ? [`${relative(REPO_ROOT, file)}:${index + 1}: ${[...new Set(matches)].join(", ")}`] : [];
  });
}

const pattern = buildPattern(loadPhrases());
const files = SURFACES.flatMap((surface) => listFiles(resolve(REPO_ROOT, surface), { extensions: SCAN_EXTENSIONS }))
  .filter(isSurfaceFile);
if (files.length === 0) {
  console.log("banned-words-static: OK — no parent-facing surfaces present yet");
  process.exit(0);
}
const hits = files.flatMap((file) => findHits(file, pattern));
if (hits.length > 0) {
  console.error(`banned-words-static: FAIL — ${hits.length} line(s) with banned words; fix the copy or add a 05 §5.3 allowlist row with its ruling`);
  for (const hit of hits) console.error(`  ${hit}`);
  process.exit(1);
}
console.log(`banned-words-static: OK — no banned words on ${files.length} static surface file(s)`);
