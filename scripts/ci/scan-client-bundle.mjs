...
function findStrings(file, needles) {
  ...
  return needles
    ...
    .map((needle) => `${relative(REPO_ROOT, file)}: ${needle}`);
}

if (!existsSync(CLIENT_CHUNKS)) {
  console.error(
    `scan-client-bundle: FAIL — ${relative(REPO_ROOT, CLIENT_CHUNKS)} not found; run 'next build' first`,
  );
  process.exit(1);
}

const chunks = listFiles(CLIENT_CHUNKS, { extensions: [".js"], skipDirs: [] });
const secretHits = chunks.flatMap((chunk) =>
  findStrings(chunk, SERVER_ONLY_NAMES),
);
const stubHits = chunks.flatMap((chunk) => findStrings(chunk, [STUB_PROVIDER]));

if (secretHits.length > 0) {
  console.error(
    "scan-client-bundle: FAIL — server-only secret name(s) present in client chunks (AC-X-36):",
  );
  for (const hit of secretHits) console.error(`  ${hit}`);
}
if (stubHits.length > 0) {
  console.error(
    `scan-client-bundle: FAIL — '${STUB_PROVIDER}' present in client chunks (AC-X-35):`,
  );
  for (const hit of stubHits) console.error(`  ${hit}`);
}
if (secretHits.length > 0 || stubHits.length > 0) process.exit(1);
console.log(
  `scan-client-bundle: OK — ${chunks.length} client chunk(s); no server-only names, no '${STUB_PROVIDER}'`,
);
